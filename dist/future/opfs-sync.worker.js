...
// src/vfs/opfs-sync.worker.ts
var root = null;
var initRoot = async () => {
  root = await navigator.storage.getDirectory();
};
var processEntry = async (entry) => {
  ...
  switch (entry.type) {
    case "write": {
      ...
      await writable.write(new Uint8Array(entry.data).buffer);
  ...
  if (type === "process-batch") {
    ...
    for (const entry of entries) {
      try {
        ...
        results.push({ path: entry.path, success: false, error: err.message });
      }
    }
