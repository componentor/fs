...
function handlePrimaryMessage(event) {
...
async function handleRequest() {
  ...
  if (requestType === REQUEST_TYPE_BINARY_ARG) {
    ...
    const jsonView = new Uint8Array(syncSAB, SYNC_DATA_OFFSET + 5, jsonLen);
    ...
    parsed.args[bufferArgIndex] = new Uint8Array(bufferView);
    request = parsed;
  } else {
    const requestDataView = new Uint8Array(syncSAB, SYNC_DATA_OFFSET, payloadLength);
...
function forwardToPrimary(request) {
...
function writeResponse(result) {
  ...
  if (result instanceof Uint8Array || result && typeof result === "object" && "type" in result && result.type === "Buffer") {
    let binaryData;
    ...
    new Uint8Array(syncSAB, SYNC_DATA_OFFSET, binaryData.length).set(binaryData);
  } else {
...
buffer/index.js:
   ...
   *)
*/
//# sourceMappingURL=fs.relay.worker.js.map