...
var enterFireAndForgetMode = () => {
  fireAndForgetDepth++;
};
var exitFireAndForgetMode = () => {
  fireAndForgetDepth = Math.max(0, fireAndForgetDepth - 1);
};
var deferredFlushMode = false;
var deferredFlushTimer = null;
var DEFERRED_FLUSH_INTERVAL_MS = VFS.DEFERRED_FLUSH_INTERVAL_MS;
var isDeferredFlushMode = () => deferredFlushMode;
...
var setVfsFileHandle = (h) => {
  vfsFileHandle = h;
};
var setVfsSyncHandle = (h) => {
  vfsSyncHandle = h;
};
var setObserver = (o) => {
  observer = o;
...
var isCompactionScheduled = () => compactionScheduled;
...
var setCompactionDebounceTimer = (timer) => {
  compactionDebounceTimer = timer;
};
var setCompactionIdleCallback = (id) => {
...
var METRIC_EXTERNAL_TOTAL = 8;
var METRIC_EXTERNAL_CREATE = 9;
var METRIC_EXTERNAL_UPDATE = 10;
var METRIC_EXTERNAL_DELETE = 11;
var METRIC_QUEUE_PATH_COUNT = 12;
var METRIC_PENDING_CREATE = 13;
var METRIC_PENDING_UPDATE = 14;
var METRIC_PENDING_DELETE = 15;
var PATH_REQUEST_FLAG = 16;
var PATH_RESPONSE_FLAG = 17;
...
var consumeEvent = (type, path, observerMtime, skipExternalCount = false) => {
  const normalized = normalizePath(path);
  const metrics = getMetrics();
  if (metrics) {
    const graceCounter = Atomics.load(metrics, RESET_GRACE_COUNTER);
  ...
  const isAlsoExternal = observerMtime !== void 0 && observerMtime > latestMtime;
...
var validateAndClearCorruptedEventsSAB = () => true;
var disableEventQueuing = () => {
};
...
// src/vfs/opfs-sync-queue.ts
var OPFS_SYNC_DEBOUNCE_MS = 100;
var pendingOpfsSync = /* @__PURE__ */ new Map();
var opfsSyncScheduled = false;
var opfsSyncDebounceTimer = null;
...
var dispatchToWorker = () => {
  ...
  for (const [path, entry] of pendingOpfsSync) {
    ...
    switch (entry.type) {
      case "write":
        opfsSyncMetrics.totalWrites++;
        break;
      case "delete":
        opfsSyncMetrics.totalDeletes++;
        break;
      case "mkdir":
        opfsSyncMetrics.totalMkdirs++;
        break;
      case "rmdir":
        opfsSyncMetrics.totalRmdirs++;
        break;
    }
  }
  ...
  const transferables = entries.filter((e) => e.data).map((e) => e.data.buffer);
  worker.postMessage(
...
var getVfsSymlinkIndex = () => vfsSymlinkIndex;

// src/vfs/files.ts
var readFromVfs = (path) => {
...
var readChunkFromVfs = (path, start, length) => {
  let normalizedPath = normalizePath(path);
  if (isSymlinkInVfs(normalizedPath)) {
    normalizedPath = resolveSymlinkInVfs(normalizedPath);
  }
...
var readFileFromVfs = (path, options) => {
...
var createDirInVfs = (path, deferSave = false) => {
  if (!path) return;
  const normalizedPath = normalizePath(path);
  if (!normalizedPath) return;
...
var deleteDirFromVfs = (path) => {
  const normalizedPath = normalizePath(path);
  vfsDirIndex.delete(normalizedPath);
  saveIndex();
  queueOpfsSync("rmdir", normalizedPath);
};
var getVfsDirIndex = () => vfsDirIndex;

// src/vfs/metadata.ts
init_buffer_shim_bundled();
var getMetadataFromVfs = (path) => {
  const normalizedPath = normalizePath(path);
...
// src/vfs/watch.ts
init_buffer_shim_bundled();
var notifyWatchListeners = (path, eventType) => {
  const normalizedPath = normalizePath(path);
...
// src/vfs/sync.ts
init_buffer_shim_bundled();
var readFileFromOpfs = async (opfsRoot, path) => {
...
var syncVfsToOpfs = async (onProgress) => {
...
var syncOpfsToVfs = async (onProgress) => {
  if (!root || !vfsSyncHandle) throw new Error("VFS not initialized");
  const files = [];
  const directories = [];
  let scannedCount = 0;
  const collectEntries = async (dir, prefix) => {
    ...
    for await (const [name, handle] of dir.entries()) {
      ...
      if (handle.kind === "file") {
        ...
        const subHasFiles = await collectEntries(handle, path);
        if (!subHasFiles) {
...
var init = async () => {
  ...
  setRoot(opfsRoot);
  const vfsFileHandle2 = await opfsRoot.getFileHandle(VFS_FILENAME, { create: true });
  setVfsFileHandle(vfsFileHandle2);
  const vfsSyncHandle2 = await vfsFileHandle2.createSyncAccessHandle();
  setVfsSyncHandle(vfsSyncHandle2);
  loadIndex();
  return;
      ...
      if (eventType === "create" || eventType === "update") {
        try {
          ...
          if (result === "synced") {
            ...
            consumeEvent(eventType, path, void 0, true);
...
var createEISDIR = (syscall, path) => new FSError("EISDIR", syscall, path);
var createEBADF = (syscall) => new FSError("EBADF", syscall);

// src/methods/readFile.ts
var navigateToFile = async (root2, path, options) => {
  const parts = path.split("/").filter((p) => p.length > 0);
...
var readFile = async (root2, path, options) => {
  const encoding = typeof options === "string" ? options : options?.encoding;
  const fileHandle = await navigateToFile(root2, path);
  const file = await fileHandle.getFile();
  const buffer = await file.arrayBuffer();
  if (encoding) {
    return new TextDecoder(encoding).decode(buffer);
...
var readFileSyncChunk = (path, start, length) => {
...
init_buffer_shim_bundled();
var writeFile = async (root2, path, data, _options) => {
...
var writeFileSync = (path, data, _options) => {
  if (path.includes("/dist/") || path.includes("/dist")) {
...
init_buffer_shim_bundled();
var mkdir = async (root2, path, options) => {
  const normalizedPath = normalizePath(path);
...
init_buffer_shim_bundled();
var rmdir = async (root2, path, options) => {
  queueEvent("delete", path);
  const parts = path.split("/").filter((p) => p.length > 0);
  let currentDir = root2;
  for (let i = 0; i < parts.length - 1; i++) {
...
var rmdirSync = (path, options) => {
  ...
  const vfsIndex2 = getVfsIndex();
  const vfsDirIndex2 = getVfsDirIndex();
  const prefix = `${normalizedPath}/`;
  const filesInDir = [];
  for (const filePath of vfsIndex2.keys()) {
    if (filePath.startsWith(prefix)) {
      filesInDir.push(filePath);
    }
  }
  const subDirs = [];
  for (const dirPath of vfsDirIndex2) {
    if (dirPath.startsWith(prefix)) {
...
init_buffer_shim_bundled();
var readdir = async (root2, path, options) => {
  const parts = path.split("/").filter((p) => p.length > 0);
  let currentDir = root2;
  for (const part of parts) {
  ...
  if (options?.recursive) {
    ...
    for (const entry of entries) {
      if (entry.kind === "directory") {
        ...
        const subEntries = await readdir(root2, subPath, { ...options, withFileTypes: false });
...
var readdirSync = (path, options) => {
  ...
  const entryTypes = /* @__PURE__ */ new Map();
  for (const filePath of vfsIndex2.keys()) {
  ...
  const vfsSymlinkIndex2 = getVfsSymlinkIndex();
  for (const symlinkPath of vfsSymlinkIndex2.keys()) {
    ...
    for (const name of entries) {
      ...
      if (type.isDir) {
        ...
        for (const subEntry of subEntries) {
          ...
          const subIsDir = vfsDirIndex2.has(subFullPath) || Array.from(vfsIndex2.keys()).some((p) => p.startsWith(subFullPath + "/"));
...
// src/methods/stat.ts
init_buffer_shim_bundled();
var FILE_EXTENSIONS2 = /\.(ts|tsx|js|jsx|mjs|cjs|mts|cts|vue|svelte|json|css|scss|sass|less|html|htm|md|txt|xml|yaml|yml|toml|wasm|map|d\.ts)$/i;
...
var stat = async (root2, path, options) => {
  const parts = path.split("/").filter((p) => p.length > 0);
...
var statSync = (path, options) => {
  ...
  const entry = vfsIndex2.get(normalizedPath);
  if (entry) {
    return createStats2(entry.size, false, false, metadata ?? void 0, bigint);
  }
  if (vfsDirIndex2.has(normalizedPath)) {
    return createStats2(0, true, false, metadata ?? void 0, bigint);
  }
  const looksLikeFile = FILE_EXTENSIONS2.test(normalizedPath);
  if (!looksLikeFile) {
    const prefix = `${normalizedPath}/`;
    for (const filePath of vfsIndex2.keys()) {
      if (filePath.startsWith(prefix)) {
...
var lstat = async (root2, path, options) => {
  return stat(root2, path, options);
};
var lstatSync = (path, options) => {
  ...
  const metadata = getMetadataFromVfs(normalizedPath);
  if (isSymlink) {
    return createStats2(0, false, true, metadata ?? void 0, bigint);
  }
...
init_buffer_shim_bundled();
var rename = async (root2, oldPath, newPath) => {
...
var copyFile = async (root2, src, dest) => {
...
init_buffer_shim_bundled();
var appendFile = async (root2, path, data) => {
  queueEvent("update", path);
  const parts = path.split("/").filter((p) => p.length > 0);
  let currentDir = root2;
  for (let i = 0; i < parts.length - 1; i++) {
...
// src/methods/rm.ts
init_buffer_shim_bundled();
var rm = async (root2, path, options) => {
  const parts = path.split("/").filter((p) => p.length > 0);
  if (parts.length === 0) {
...
var rmSync = (path, options) => {
  ...
  const vfsDirIndex2 = getVfsDirIndex();
  const normalizedPath = normalizePath(path);
  if (vfsIndex2.has(normalizedPath)) {
    queueEvent("delete", normalizedPath);
...
var access = async (root2, path, mode = constants.F_OK) => {
  const parts = path.split("/").filter((p) => p.length > 0);
  if (parts.length === 0) {
    return;
  }
...
var accessSync = (path, mode = constants.F_OK) => {
  const normalizedPath = normalizePath(path);
  if (normalizedPath === "") return;
...
var lchownSync = (_path, _uid, _gid) => {
};

// src/methods/link.ts
init_buffer_shim_bundled();
var link = async (root2, existingPath, newPath) => {
  queueEvent("create", newPath);
...
var linkSync = (existingPath, newPath) => {
  ...
  const vfsIndex2 = getVfsIndex();
  const srcEntry = vfsIndex2.get(normalizedSrc);
  if (!srcEntry) {
    throw createENOENT("link", existingPath);
  }
  vfsIndex2.set(normalizedDest, { ...srcEntry });
};
...
var symlinkSync = (target, path, _type) => {
  const normalizedPath = normalizePath(path);
  createSymlinkInVfs(normalizedPath, target);
};
...
init_buffer_shim_bundled();
var truncate = async (root2, path, len = 0) => {
  queueEvent("update", path);
  const parts = path.split("/").filter((p) => p.length > 0);
  let currentDir = root2;
  for (let i = 0; i < parts.length - 1; i++) {
...
var mkdtemp = async (root2, prefix) => {
...
var lutimesSync = (_path, _atime, _mtime) => {
};

// src/methods/cp.ts
init_buffer_shim_bundled();
var cp = async (root2, src, dest, options) => {
  queueEvent("create", dest);
  const srcParts = src.split("/").filter((p) => p.length > 0);
  const destParts = dest.split("/").filter((p) => p.length > 0);
  let srcDir = root2;
...
var cpSync = (src, dest, options) => {
  ...
  const isDir = vfsDirIndex2.has(normalizedSrc) || Array.from(vfsIndex2.keys()).some((p) => p.startsWith(srcPrefix));
  if (!isDir) {
...
var openSync = (path, flags = "r", _mode) => {
  const normalizedPath = normalizePath(path);
...
// src/methods/read.ts
init_buffer_shim_bundled();
var readSync = (fd, buffer, offsetOrOptions, length, position) => {
  const entry = getFdEntry(fd);
  if (!entry) {
...
init_buffer_shim_bundled();
var writeSync = (fd, bufferOrString, offsetOrPositionOrOptions, lengthOrEncoding, position) => {
  const entry = getFdEntry(fd);
  if (!entry) {
...
var fstatSync = (fd) => {
  ...
  return createStats3(content?.length || 0, isDirectoryInVfs(entry.path));
};
...
init_buffer_shim_bundled();
var fchmodSync = (_fd, _mode) => {
};

// src/methods/fchown.ts
init_buffer_shim_bundled();
var fchownSync = (_fd, _uid, _gid) => {
};

// src/methods/futimes.ts
init_buffer_shim_bundled();
var futimesSync = (_fd, _atime, _mtime) => {
};

// src/methods/readv.ts
init_buffer_shim_bundled();
var readvSync = (fd, buffers, position) => {
  const entry = getFdEntry(fd);
  if (!entry) {
...
init_buffer_shim_bundled();
var writevSync = (fd, buffers, position) => {
  const entry = getFdEntry(fd);
  if (!entry) {
...
// src/fs.sync.queue.ts
init_buffer_shim_bundled();
var MAX_ASYNC_QUEUE_SIZE = 1e3;
var asyncQueueMetrics = {
  totalQueued: 0,
  totalProcessed: 0,
  totalDropped: 0,
  peakQueueSize: 0
...
var enqueueFireAndForget = (method, args) => {
...
var logLevel = "info";
var nextLogId = 1;
var setLogging = (enabled, level) => {
  loggingEnabled = enabled;
  logLevel = level;
};
function log(method, args, phase, duration, error) {
  ...
  if (phase === "START" && logLevel === "verbose") {
    ...
    const durStr = duration !== void 0 ? ` (${duration.toFixed(2)}ms)` : "";
    const status = error ? "\u2717" : "\u2713";
...
var startRequestLoop = (syncSAB2, storageMode2) => {
  ...
  const processLoop = () => {
    ...
    while (true) {
      ...
      const requestType = new Uint8Array(syncSAB2, SYNC_TYPE_OFFSET, 1)[0];
      const payloadLength = lengthView.getUint32(0);
      if (payloadLength > 60 * 1024 * 1024) {
      ...
      try {
        if (requestType === REQUEST_TYPE_BINARY_ARG) {
          const headerView = new DataView(syncSAB2, SYNC_DATA_OFFSET, 5);
          ...
          const jsonData = new Uint8Array(syncSAB2, SYNC_DATA_OFFSET + 5, jsonLen).slice();
          ...
          if (bufferLen > 0) {
            const bufferView = new Uint8Array(syncSAB2, bufferStart, bufferLen);
            args[bufferArgIndex] = Buffer.from(new Uint8Array(bufferView));
          }
        } else {
          const requestData = new Uint8Array(syncSAB2, SYNC_DATA_OFFSET, payloadLength).slice();
      ...
      try {
        ...
        if (asyncVfsFn) {
          ...
          return;
        } else if (storageMode2 === "opfs-only") {
          const fn = opfsMethods[method];
          if (!fn) throw new Error(`Unknown method: ${method}`);
          const root2 = getRoot();
          if (!root2) throw new Error("OPFS root not initialized");
          (async () => {
            try {
              result = await fn(root2, ...args);
              writeResponse(result);
            } catch (err) {
              writeError(err.message);
            }
        ...
        log(method, args, "END", performance.now() - startTime, err.message);
      }
      const typeView = new Uint8Array(syncSAB2, SYNC_TYPE_OFFSET, 1);
      typeView[0] = responseType;
      lengthView.setUint32(0, responseData.length);
      new Uint8Array(syncSAB2, SYNC_DATA_OFFSET, responseData.length).set(responseData);
...
buffer/index.js:
   ...
   *)
*/
//# sourceMappingURL=fs.sync.worker.js.map