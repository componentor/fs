...
var sync_relay_default = 'var lt=1447449377,At=1,Dt=4096,Pt=1e5,P=64,b={SIZE:64,MAGIC:0,VERSION:4,INODE_COUNT:8,BLOCK_SIZE:12,TOTAL_BLOCKS:16,FREE_BLOCKS:20,INODE_OFFSET:24,PATH_OFFSET:32,DATA_OFFSET:40,BITMAP_OFFSET:48,PATH_USED:56,CRC32:60},I={TYPE:0,FLAGS:1,PATH_OFFSET:4,PATH_LENGTH:8,NLINK:10,MODE:12,SIZE:16,FIRST_BLOCK:24,BLOCK_COUNT:28,MTIME:32,CTIME:40,ATIME:48,UID:56,GID:60},p={FREE:0,FILE:1,DIRECTORY:2,SYMLINK:3},Ue=33188,Xt=16877,xe=41471,jt=18,Jt=61440,Pe=32768,Le=16384,Lt=40,te=256*1024,Mt=1024,ee=4e6;function Me(i=Pt,t=Dt,e=Mt,s=ee){const n=b.SIZE,r=i*P,a=n+r,o=te,l=a+o,f=Math.ceil(s/8),h=Math.ceil(e/8),d=Math.ceil((l+f)/t)*t,m=d+e*t;return{inodeTableOffset:n,inodeTableSize:r,pathTableOffset:a,pathTableSize:o,bitmapOffset:l,bitmapSize:h,bitmapRegionSize:f,dataOffset:d,totalSize:m,totalBlocks:e}}var Ne=(()=>{const i=new Uint32Array(256);for(let t=0;t<256;t++){let e=t;for(let s=0;s<8;s++)e=e&1?3988292384^e>>>1:e>>>1;i[t]=e>>>0}return i})();function se(i,t=0,e=i.byteLength){let s=4294967295;for(let n=t;n<e;n++)s=Ne[(s^i[n])&255]^s>>>8;return(s^4294967295)>>>0}var y={OK:0,ENOENT:1,EEXIST:2,EISDIR:3,ENOTDIR:4,ENOTEMPTY:5,EACCES:6,EINVAL:7,EBADF:8,ELOOP:9,ENOSPC:10,EIO:11,ENOTSUP:12},V=new TextEncoder,Nt=16384,ut=new TextDecoder,Be=class Tt{handle;pathIndex=new Map;inodeCount=0;blockSize=Dt;totalBlocks=0;freeBlocks=0;inodeTableOffset=0;pathTableOffset=0;pathTableUsed=0;pathTableSize=0;bitmapOffset=0;dataOffset=0;umask=jt;processUid=0;processGid=0;strictPermissions=!1;debug=!1;fdTable=new Map;nextFd=3;static isReadable(t){const e=t&3;return e===0||e===2}static isWritable(t){const e=t&3;return e===1||e===2}inodeBuf=new Uint8Array(P);inodeView=new DataView(this.inodeBuf.buffer);inodeCache=new Map;superblockBuf=new Uint8Array(b.SIZE);superblockView=new DataView(this.superblockBuf.buffer);bitmap=null;bitmapDirtyLo=1/0;bitmapDirtyHi=-1;superblockDirty=!1;freeInodeHint=0;implicitDirs=new Map;implicitDirsGen=-1;pathIndexGen=0;descCount=new Map;descCountGen=0;childIndex=new Map;childIndexGen=0;allocCursor=0;symlinkLoopDetected=!1;resolveFailureStatus(){return this.symlinkLoopDetected?y.ELOOP:y.ENOENT}maxInodes=4e6;maxBlocks=ee;maxPathTable=256*1024*1024;maxVFSSize=100*1024*1024*1024;init(t,e){if(this.handle=t,this.processUid=e?.uid??0,this.processGid=e?.gid??0,this.umask=e?.umask??jt,this.strictPermissions=e?.strictPermissions??!1,this.debug=e?.debug??!1,e?.limits&&(e.limits.maxInodes!=null&&(this.maxInodes=e.limits.maxInodes),e.limits.maxBlocks!=null&&(this.maxBlocks=e.limits.maxBlocks),e.limits.maxPathTable!=null&&(this.maxPathTable=e.limits.maxPathTable),e.limits.maxVFSSize!=null&&(this.maxVFSSize=e.limits.maxVFSSize)),t.getSize()===0)this.format();else try{this.mount()}catch(n){const r=n.message??String(n);throw r.startsWith("Corrupt VFS:")?n:new Error(`Corrupt VFS: ${r}`)}}closeHandle(){try{this.handle?.close()}catch{}}format(){const t=Me(Pt,Dt,Mt,this.maxBlocks);this.inodeCount=Pt,this.blockSize=Dt,this.totalBlocks=t.totalBlocks,this.freeBlocks=t.totalBlocks,this.inodeTableOffset=t.inodeTableOffset,this.pathTableOffset=t.pathTableOffset,this.pathTableSize=t.pathTableSize,this.pathTableUsed=0,this.bitmapOffset=t.bitmapOffset,this.dataOffset=t.dataOffset,this.handle.truncate(t.totalSize),this.writeSuperblock();const e=new Uint8Array(t.inodeTableSize);this.handle.write(e,{at:this.inodeTableOffset}),this.bitmap=new Uint8Array(t.bitmapSize),this.handle.write(this.bitmap,{at:this.bitmapOffset}),this.createInode("/",p.DIRECTORY,Xt,0),this.writeSuperblock(),this.handle.flush()}mount(){const t=this.handle.getSize();if(t<b.SIZE)throw new Error(`Corrupt VFS: file too small (${t} bytes, need at least ${b.SIZE})`);this.handle.read(this.superblockBuf,{at:0});const e=this.superblockView,s=e.getUint32(b.MAGIC,!0);if(s!==lt)throw new Error(`Corrupt VFS: bad magic 0x${s.toString(16)} (expected 0x${lt.toString(16)})`);const n=e.getUint32(b.VERSION,!0);if(n!==At)throw new Error(`Corrupt VFS: unsupported version ${n} (expected ${At})`);const r=e.getUint32(b.CRC32,!0);if(r!==0){const R=se(this.superblockBuf,0,b.CRC32);if(R!==r)throw new Error(`Corrupt VFS: superblock checksum mismatch (stored 0x${r.toString(16)}, computed 0x${R.toString(16)})`)}const a=e.getUint32(b.INODE_COUNT,!0),o=e.getUint32(b.BLOCK_SIZE,!0),l=e.getUint32(b.TOTAL_BLOCKS,!0),f=e.getUint32(b.FREE_BLOCKS,!0),h=e.getFloat64(b.INODE_OFFSET,!0),d=e.getFloat64(b.PATH_OFFSET,!0),m=e.getFloat64(b.DATA_OFFSET,!0),w=e.getFloat64(b.BITMAP_OFFSET,!0),u=e.getUint32(b.PATH_USED,!0);if(o===0||(o&o-1)!==0)throw new Error(`Corrupt VFS: invalid block size ${o} (must be power of 2)`);if(a===0)throw new Error("Corrupt VFS: inode count is 0");if(f>l)throw new Error(`Corrupt VFS: free blocks (${f}) exceeds total blocks (${l})`);if(a>this.maxInodes)throw new Error(`Corrupt VFS: inode count ${a} exceeds maximum ${this.maxInodes}`);if(l>this.maxBlocks)throw new Error(`Corrupt VFS: total blocks ${l} exceeds maximum ${this.maxBlocks}`);if(t>this.maxVFSSize)throw new Error(`Corrupt VFS: file size ${t} exceeds maximum ${this.maxVFSSize}`);if(!Number.isFinite(h)||h<0||!Number.isFinite(d)||d<0||!Number.isFinite(w)||w<0||!Number.isFinite(m)||m<0)throw new Error("Corrupt VFS: non-finite or negative section offset");if(h!==b.SIZE)throw new Error(`Corrupt VFS: inode table offset ${h} (expected ${b.SIZE})`);const E=h+a*P;if(d!==E)throw new Error(`Corrupt VFS: path table offset ${d} (expected ${E})`);if(w<=d)throw new Error(`Corrupt VFS: bitmap offset ${w} must be after path table ${d}`);if(m<=w)throw new Error(`Corrupt VFS: data offset ${m} must be after bitmap ${w}`);if(l>(m-w)*8)throw new Error(`Corrupt VFS: total blocks (${l}) exceed bitmap region capacity (${(m-w)*8})`);const D=w-d;if(u>D)throw new Error(`Corrupt VFS: path used (${u}) exceeds path table size (${D})`);if(D>this.maxPathTable)throw new Error(`Corrupt VFS: path table size ${D} exceeds maximum ${this.maxPathTable}`);const C=m+l*o;if(C>this.maxVFSSize)throw new Error(`Corrupt VFS: computed layout size ${C} exceeds maximum ${this.maxVFSSize}`);if(t<C)throw new Error(`Corrupt VFS: file size ${t} too small for layout (need ${C})`);this.inodeCount=a,this.blockSize=o,this.totalBlocks=l,this.freeBlocks=f,this.inodeTableOffset=h,this.pathTableOffset=d,this.dataOffset=m,this.bitmapOffset=w,this.pathTableUsed=u,this.pathTableSize=D;const F=Math.ceil(this.totalBlocks/8);if(this.bitmap=new Uint8Array(F),this.handle.read(this.bitmap,{at:this.bitmapOffset}),this.rebuildIndex(),!this.pathIndex.has("/"))throw new Error(\'Corrupt VFS: root directory "/" not found in inode table\')}writeSuperblock(){const t=this.superblockView;t.setUint32(b.MAGIC,lt,!0),t.setUint32(b.VERSION,At,!0),t.setUint32(b.INODE_COUNT,this.inodeCount,!0),t.setUint32(b.BLOCK_SIZE,this.blockSize,!0),t.setUint32(b.TOTAL_BLOCKS,this.totalBlocks,!0),t.setUint32(b.FREE_BLOCKS,this.freeBlocks,!0),t.setFloat64(b.INODE_OFFSET,this.inodeTableOffset,!0),t.setFloat64(b.PATH_OFFSET,this.pathTableOffset,!0),t.setFloat64(b.DATA_OFFSET,this.dataOffset,!0),t.setFloat64(b.BITMAP_OFFSET,this.bitmapOffset,!0),t.setUint32(b.PATH_USED,this.pathTableUsed,!0),t.setUint32(b.CRC32,se(this.superblockBuf,0,b.CRC32),!0),this.handle.write(this.superblockBuf,{at:0})}markBitmapDirty(t,e){t<this.bitmapDirtyLo&&(this.bitmapDirtyLo=t),e>this.bitmapDirtyHi&&(this.bitmapDirtyHi=e)}commitPending(){if(this.blocksFreedsinceTrim&&(this.trimTrailingBlocks(),this.blocksFreedsinceTrim=!1),this.bitmapDirtyHi>=0){const t=this.bitmapDirtyLo,e=this.bitmapDirtyHi;this.handle.write(this.bitmap.subarray(t,e+1),{at:this.bitmapOffset+t}),this.bitmapDirtyLo=1/0,this.bitmapDirtyHi=-1}this.superblockDirty&&(this.writeSuperblock(),this.superblockDirty=!1)}findLastUsedBlock(){const t=this.bitmap;for(let e=Math.ceil(this.totalBlocks/8)-1;e>=0;e--)if(t[e]!==0)for(let s=7;s>=0;s--){const n=e*8+s;if(n<this.totalBlocks&&t[e]&1<<s)return n}return-1}trimTrailingBlocks(){const t=this.findLastUsedBlock(),e=Math.max(t+1+Nt,Mt);if(e>=this.totalBlocks)return;this.handle.truncate(this.dataOffset+e*this.blockSize);const s=Math.ceil(e/8);this.bitmap=this.bitmap.slice(0,s);const n=this.totalBlocks-e;this.freeBlocks-=n,this.totalBlocks=e,this.superblockDirty=!0,this.bitmapDirtyLo=0,this.bitmapDirtyHi=s-1}lastPreGrowCheck=0;maybePreGrow(t=!1){if(!this.bitmap)return!1;const e=Date.now();if(!t&&e-this.lastPreGrowCheck<250)return!1;this.lastPreGrowCheck=e;const s=this.totalBlocks-(this.findLastUsedBlock()+1);if(s>=Nt)return!1;const n=Math.min(this.maxBlocks,this.bitmapCapacityBlocks()),r=Math.ceil((Nt-s)/8)*8,a=Math.min(r,n-this.totalBlocks);if(a<=0)return!1;const o=this.totalBlocks+a;this.handle.truncate(this.dataOffset+o*this.blockSize);const l=Math.ceil(o/8);if(l>this.bitmap.byteLength){const f=new Uint8Array(l);f.set(this.bitmap),this.bitmap=f}return this.totalBlocks=o,this.freeBlocks+=a,this.superblockDirty=!0,this.commitPending(),!0}rebuildIndex(){this.pathIndex.clear(),this.inodeCache.clear();const t=this.inodeCount*P,e=new Uint8Array(t);this.handle.read(e,{at:this.inodeTableOffset});const s=new DataView(e.buffer),n=this.pathTableUsed>0?new Uint8Array(this.pathTableUsed):null;n&&this.handle.read(n,{at:this.pathTableOffset});for(let r=0;r<this.inodeCount;r++){const a=r*P,o=s.getUint8(a+I.TYPE);if(o===p.FREE)continue;if(o<p.FILE||o>p.SYMLINK)throw new Error(`Corrupt VFS: inode ${r} has invalid type ${o}`);const l=s.getUint32(a+I.PATH_OFFSET,!0),f=s.getUint16(a+I.PATH_LENGTH,!0),h=s.getFloat64(a+I.SIZE,!0),d=s.getUint32(a+I.FIRST_BLOCK,!0),m=s.getUint32(a+I.BLOCK_COUNT,!0);if(f===0||l+f>this.pathTableUsed)throw new Error(`Corrupt VFS: inode ${r} path out of bounds (offset=${l}, len=${f}, tableUsed=${this.pathTableUsed})`);if(o!==p.DIRECTORY){if(h<0||!isFinite(h))throw new Error(`Corrupt VFS: inode ${r} has invalid size ${h}`);if(m>0&&d+m>this.totalBlocks)throw new Error(`Corrupt VFS: inode ${r} data blocks out of range (first=${d}, count=${m}, total=${this.totalBlocks})`)}const w={type:o,pathOffset:l,pathLength:f,nlink:s.getUint16(a+I.NLINK,!0)||1,mode:s.getUint32(a+I.MODE,!0),size:h,firstBlock:d,blockCount:m,mtime:s.getFloat64(a+I.MTIME,!0),ctime:s.getFloat64(a+I.CTIME,!0),atime:s.getFloat64(a+I.ATIME,!0),uid:s.getUint32(a+I.UID,!0),gid:s.getUint32(a+I.GID,!0)};this.inodeCache.set(r,w);let u;if(n?u=ut.decode(n.subarray(w.pathOffset,w.pathOffset+w.pathLength)):u=this.readPath(w.pathOffset,w.pathLength),!u.startsWith("/")||u.includes("\\0"))throw new Error(`Corrupt VFS: inode ${r} has invalid path "${u.substring(0,50)}"`);this.setPathIndex(u,r)}this.pathIndexGen++}readInode(t){const e=this.inodeCache.get(t);if(e)return e;const s=this.inodeTableOffset+t*P;this.handle.read(this.inodeBuf,{at:s});const n=this.inodeView,r={type:n.getUint8(I.TYPE),pathOffset:n.getUint32(I.PATH_OFFSET,!0),pathLength:n.getUint16(I.PATH_LENGTH,!0),nlink:n.getUint16(I.NLINK,!0)||1,mode:n.getUint32(I.MODE,!0),size:n.getFloat64(I.SIZE,!0),firstBlock:n.getUint32(I.FIRST_BLOCK,!0),blockCount:n.getUint32(I.BLOCK_COUNT,!0),mtime:n.getFloat64(I.MTIME,!0),ctime:n.getFloat64(I.CTIME,!0),atime:n.getFloat64(I.ATIME,!0),uid:n.getUint32(I.UID,!0),gid:n.getUint32(I.GID,!0)};return this.inodeCache.set(t,r),r}writeInode(t,e){e.type===p.FREE?this.inodeCache.delete(t):this.inodeCache.set(t,e);const s=this.inodeView;s.setUint8(I.TYPE,e.type),s.setUint8(I.FLAGS,0),s.setUint8(I.FLAGS+1,0),s.setUint8(I.FLAGS+2,0),s.setUint32(I.PATH_OFFSET,e.pathOffset,!0),s.setUint16(I.PATH_LENGTH,e.pathLength,!0),s.setUint16(I.NLINK,e.nlink,!0),s.setUint32(I.MODE,e.mode,!0),s.setFloat64(I.SIZE,e.size,!0),s.setUint32(I.FIRST_BLOCK,e.firstBlock,!0),s.setUint32(I.BLOCK_COUNT,e.blockCount,!0),s.setFloat64(I.MTIME,e.mtime,!0),s.setFloat64(I.CTIME,e.ctime,!0),s.setFloat64(I.ATIME,e.atime,!0),s.setUint32(I.UID,e.uid,!0),s.setUint32(I.GID,e.gid,!0);const n=this.inodeTableOffset+t*P;this.handle.write(this.inodeBuf,{at:n})}readPath(t,e){const s=new Uint8Array(e);return this.handle.read(s,{at:this.pathTableOffset+t}),ut.decode(s)}appendPath(t){const e=V.encode(t),s=this.pathTableUsed;return s+e.byteLength>this.pathTableSize&&this.growPathTable(s+e.byteLength),this.handle.write(e,{at:this.pathTableOffset+s}),this.pathTableUsed+=e.byteLength,this.superblockDirty=!0,{offset:s,length:e.byteLength}}growPathTable(t){const e=Math.max(this.pathTableSize*2,t+te),s=e-this.pathTableSize,n=this.handle.getSize()+s;this.handle.truncate(n);const r=this.totalBlocks*this.blockSize,a=4*1024*1024,o=new Uint8Array(Math.min(a,Math.max(r,1)));let l=r;for(;l>0;){const d=Math.min(l,a),m=this.dataOffset+(l-d),w=this.dataOffset+s+(l-d),u=d<o.length?o.subarray(0,d):o;this.handle.read(u,{at:m}),this.handle.write(u,{at:w}),l-=d}const f=this.bitmapOffset+s,h=this.dataOffset+s;this.handle.write(this.bitmap,{at:f}),this.pathTableSize=e,this.bitmapOffset=f,this.dataOffset=h,this.superblockDirty=!0}zeroFileRange(t,e,s=1/0){if(e<=0)return;const n=Math.min(t+e,s);if(n<=t)return;const r=4*1024*1024,a=n-t,o=new Uint8Array(Math.min(a,r));let l=0;for(;l<a;){const f=Math.min(r,a-l),h=f<o.length?o.subarray(0,f):o;this.handle.write(h,{at:t+l}),l+=f}}allocateBlocks(t){if(t===0)return 0;let e=this.scanForRun(this.allocCursor,this.totalBlocks,t);if(e<0&&this.allocCursor>0){const r=Math.min(this.allocCursor+t-1,this.totalBlocks);e=this.scanForRun(0,r,t)}if(e<0)return this.growAndAllocate(t);const s=e+t-1,n=this.bitmap;for(let r=e;r<=s;r++)n[r>>>3]|=1<<(r&7);return this.markBitmapDirty(e>>>3,s>>>3),this.freeBlocks-=t,this.superblockDirty=!0,this.allocCursor=s+1>=this.totalBlocks?0:s+1,e}scanForRun(t,e,s){const n=this.bitmap;let r=0,a=t;for(let o=t;o<e;o++){if(r===0&&(o&7)===0&&n[o>>>3]===255){o+=7,a=o+1;continue}if(n[o>>>3]>>>(o&7)&1)r=0,a=o+1;else if(++r===s)return a}return-1}bitmapCapacityBlocks(){return(this.dataOffset-this.bitmapOffset)*8}growAndAllocate(t){const e=this.totalBlocks,s=Math.min(this.maxBlocks,this.bitmapCapacityBlocks());let n=Math.max(e*2,e+t);if(n>s&&(n=s),n<e+t)throw new Error(`ENOSPC: cannot allocate ${t} blocks (total ${e}, ceiling ${s})`);const r=n-e,a=this.dataOffset+n*this.blockSize;this.handle.truncate(a);const o=Math.ceil(n/8),l=new Uint8Array(o);l.set(this.bitmap),this.bitmap=l,this.totalBlocks=n,this.freeBlocks+=r;const f=e;for(let h=f;h<f+t;h++){const d=h>>>3,m=h&7;this.bitmap[d]|=1<<m}return this.markBitmapDirty(f>>>3,f+t-1>>>3),this.freeBlocks-=t,this.superblockDirty=!0,f}blocksFreedsinceTrim=!1;freeBlockRange(t,e){if(e===0)return;const s=this.bitmap;for(let n=t;n<t+e;n++){const r=n>>>3,a=n&7;s[r]&=~(1<<a)}this.markBitmapDirty(t>>>3,t+e-1>>>3),this.freeBlocks+=e,this.superblockDirty=!0,this.blocksFreedsinceTrim=!0}findFreeInode(){for(let e=this.freeInodeHint;e<this.inodeCount;e++){if(this.inodeCache.has(e))continue;const s=this.inodeTableOffset+e*P,n=new Uint8Array(1);if(this.handle.read(n,{at:s}),n[0]===p.FREE)return this.freeInodeHint=e+1,e}const t=this.growInodeTable();return this.freeInodeHint=t+1,t}growInodeTable(){const t=this.inodeCount,e=t*2,s=(e-t)*P,n=this.inodeTableOffset+t*P,r=this.handle.getSize(),a=r-n;this.handle.truncate(r+s);const o=8*1024*1024;if(a>0){const d=new Uint8Array(Math.min(o,a));let m=a;for(;m>0;){const w=Math.min(o,m),u=n+m-w,E=w===d.length?d:d.subarray(0,w);this.handle.read(E,{at:u}),this.handle.write(E,{at:u+s}),m-=w}}const l=new Uint8Array(Math.min(o,s));let f=s,h=n;for(;f>0;){const d=Math.min(o,f);this.handle.write(d===l.length?l:l.subarray(0,d),{at:h}),h+=d,f-=d}return this.pathTableOffset+=s,this.bitmapOffset+=s,this.dataOffset+=s,this.inodeCount=e,this.superblockDirty=!0,t}readData(t,e,s){const n=new Uint8Array(s),r=this.dataOffset+t*this.blockSize;return this.handle.read(n,{at:r}),n}writeData(t,e){const s=this.dataOffset+t*this.blockSize;this.handle.write(e,{at:s})}resolvePath(t,e=0){if(e===0&&(this.symlinkLoopDetected=!1),e>Lt){this.symlinkLoopDetected=!0;return}const s=this.pathIndex.get(t);if(s===void 0)return this.resolvePathComponents(t,!0,e);const n=this.readInode(s);if(n.type===p.SYMLINK){const r=ut.decode(this.readData(n.firstBlock,n.blockCount,n.size)),a=r.startsWith("/")?r:this.resolveRelative(t,r);return this.resolvePath(a,e+1)}return s}resolvePathComponents(t,e=!0,s=0){return this.resolvePathFull(t,e,s)?.idx}resolvePathFull(t,e=!0,s=0){if(s===0&&(this.symlinkLoopDetected=!1),s>Lt){this.symlinkLoopDetected=!0;return}const n=t.split("/").filter(Boolean);let r="/";for(let o=0;o<n.length;o++){const l=o===n.length-1;r=r==="/"?"/"+n[o]:r+"/"+n[o];const f=this.pathIndex.get(r);if(f===void 0)return;const h=this.readInode(f);if(h.type===p.SYMLINK&&(!l||e)){const d=ut.decode(this.readData(h.firstBlock,h.blockCount,h.size)),m=d.startsWith("/")?d:this.resolveRelative(r,d);if(l)return this.resolvePathFull(m,!0,s+1);const w=n.slice(o+1).join("/"),u=m+(w?"/"+w:"");return this.resolvePathFull(u,e,s+1)}}const a=this.pathIndex.get(r);if(a!==void 0)return{idx:a,resolvedPath:r}}resolveDanglingLink(t,e=0){if(e>Lt)return null;const s=this.pathIndex.get(t);if(s===void 0)return t;const n=this.readInode(s);if(n.type!==p.SYMLINK)return t;const r=ut.decode(this.readData(n.firstBlock,n.blockCount,n.size)),a=r.startsWith("/")?r:this.resolveRelative(t,r);return this.resolveDanglingLink(a,e+1)}resolveRelative(t,e){const n=((t.substring(0,t.lastIndexOf("/"))||"/")+"/"+e).split("/").filter(Boolean),r=[];for(const a of n)if(a!=="."){if(a===".."){r.pop();continue}r.push(a)}return"/"+r.join("/")}createInode(t,e,s,n,r){const a=this.findFreeInode(),{offset:o,length:l}=this.appendPath(t),f=Date.now();let h=0,d=0;r&&r.byteLength>0&&(d=Math.ceil(r.byteLength/this.blockSize),h=this.allocateBlocks(d),this.writeData(h,r));const m={type:e,pathOffset:o,pathLength:l,nlink:e===p.DIRECTORY?2:1,mode:s,size:n,firstBlock:h,blockCount:d,mtime:f,ctime:f,atime:f,uid:this.processUid,gid:this.processGid};return this.writeInode(a,m),this.setPathIndex(t,a),this.pathIndexGen++,a}normalizePath(t){if(t.charCodeAt(0)!==47&&(t="/"+t),t.length===1||t.indexOf("/.")===-1&&t.indexOf("//")===-1&&t.charCodeAt(t.length-1)!==47)return t;const e=t.split("/").filter(Boolean),s=[];for(const n of e)if(n!=="."){if(n===".."){s.pop();continue}s.push(n)}return"/"+s.join("/")}read(t){const e=this.debug?performance.now():0;t=this.normalizePath(t);let s=this.pathIndex.get(t);if(s!==void 0){const a=this.inodeCache.get(s);if(a)if(a.type===p.SYMLINK)s=this.resolvePathComponents(t,!0);else{if(a.type===p.DIRECTORY)return{status:y.EISDIR,data:null};{const o=a.size>0?this.readData(a.firstBlock,a.blockCount,a.size):new Uint8Array(0);if(this.debug){const l=performance.now();console.log(`[VFS read] path=${t} size=${a.size} TOTAL=${(l-e).toFixed(3)}ms (fast)`)}return{status:0,data:o}}}}if(s===void 0&&(s=this.resolvePathComponents(t,!0)),s===void 0)return{status:this.resolveFailureStatus(),data:null};const n=this.readInode(s);if(n.type===p.DIRECTORY)return{status:y.EISDIR,data:null};const r=n.size>0?this.readData(n.firstBlock,n.blockCount,n.size):new Uint8Array(0);if(this.debug){const a=performance.now();console.log(`[VFS read] path=${t} size=${n.size} TOTAL=${(a-e).toFixed(3)}ms (slow path)`)}return{status:0,data:r}}write(t,e,s=0){const n=this.debug?performance.now():0;t=this.normalizePath(t);const r=this.debug?performance.now():0,a=this.ensureParent(t);if(a!==0)return{status:a};const o=this.debug?performance.now():0;let l=this.resolvePathComponents(t,!0);if(l===void 0){const u=this.resolveDanglingLink(t);if(u===null)return{status:y.ELOOP};if(u!==t){t=u;const E=this.ensureParent(t);if(E!==0)return{status:E};l=this.resolvePathComponents(t,!0)}}const f=this.debug?performance.now():0;let h=f,d=f,m=f;if(l!==void 0){const u=this.readInode(l);if(u.type===p.DIRECTORY)return{status:y.EISDIR};const E=Math.ceil(e.byteLength/this.blockSize);if(E<=u.blockCount)h=this.debug?performance.now():0,this.writeData(u.firstBlock,e),d=this.debug?performance.now():0,E<u.blockCount&&this.freeBlockRange(u.firstBlock+E,u.blockCount-E);else{this.freeBlockRange(u.firstBlock,u.blockCount);const D=this.allocateBlocks(E);h=this.debug?performance.now():0,this.writeData(D,e),d=this.debug?performance.now():0,u.firstBlock=D}u.size=e.byteLength,u.blockCount=E,u.mtime=Date.now(),this.writeInode(l,u),m=this.debug?performance.now():0}else{if(this.isImplicitDirectory(t))return{status:y.EISDIR};const u=Ue&~(this.umask&511);this.createInode(t,p.FILE,u,e.byteLength,e),h=this.debug?performance.now():0,d=h,m=h}this.commitPending(),s&1&&this.handle.flush();const w=this.debug?performance.now():0;if(this.debug){const u=l!==void 0;console.log(`[VFS write] path=${t} size=${e.byteLength} ${u?"UPDATE":"CREATE"} normalize=${(r-n).toFixed(3)}ms parent=${(o-r).toFixed(3)}ms resolve=${(f-o).toFixed(3)}ms alloc=${(h-f).toFixed(3)}ms data=${(d-h).toFixed(3)}ms inode=${(m-d).toFixed(3)}ms flush=${(w-m).toFixed(3)}ms TOTAL=${(w-n).toFixed(3)}ms`)}return{status:0}}append(t,e){t=this.normalizePath(t);const s=this.resolvePathComponents(t,!0);if(s===void 0)return this.write(t,e);const n=this.readInode(s);if(n.type===p.DIRECTORY)return{status:y.EISDIR};const r=n.size+e.byteLength,a=Math.ceil(r/this.blockSize);if(a<=n.blockCount)return this.handle.write(e,{at:this.dataOffset+n.firstBlock*this.blockSize+n.size}),n.size=r,n.mtime=Date.now(),this.writeInode(s,n),this.commitPending(),{status:0};const o=this.allocateBlocks(a),l=this.dataOffset+o*this.blockSize;if(n.size>0){const f=this.dataOffset+n.firstBlock*this.blockSize,h=4*1024*1024,d=new Uint8Array(Math.min(h,n.size));let m=0;for(;m<n.size;){const w=Math.min(h,n.size-m),u=w<d.length?d.subarray(0,w):d;this.handle.read(u,{at:f+m}),this.handle.write(u,{at:l+m}),m+=w}}return this.freeBlockRange(n.firstBlock,n.blockCount),this.handle.write(e,{at:l+n.size}),n.firstBlock=o,n.blockCount=a,n.size=r,n.mtime=Date.now(),this.writeInode(s,n),this.commitPending(),{status:0}}unlink(t){t=this.normalizePath(t);const e=this.pathIndex.get(t);if(e===void 0)return{status:y.ENOENT};const s=this.readInode(e);return s.type===p.DIRECTORY?{status:y.EISDIR}:(s.nlink=Math.max(0,s.nlink-1),this.freeBlockRange(s.firstBlock,s.blockCount),s.type=p.FREE,this.writeInode(e,s),this.deletePathIndex(t),this.pathIndexGen++,e<this.freeInodeHint&&(this.freeInodeHint=e),this.commitPending(),{status:0})}stat(t){t=this.normalizePath(t);const e=this.resolvePathComponents(t,!0);if(e===void 0){const s=this.resolveFailureStatus();return this.isImplicitDirectory(t)?this.encodeImplicitDirStatResponse(t):{status:s,data:null}}return this.encodeStatResponse(e)}lstat(t){t=this.normalizePath(t);let e=this.resolvePathComponents(t,!1);return e===void 0&&(e=this.resolvePathComponents(t,!0),e===void 0)?this.isImplicitDirectory(t)?this.encodeImplicitDirStatResponse(t):{status:y.ENOENT,data:null}:this.encodeStatResponse(e)}encodeStatResponse(t){const e=this.readInode(t);let s=e.nlink;e.type===p.DIRECTORY&&(s=2+this.countSubdirectories(this.readPath(e.pathOffset,e.pathLength)));const n=new Uint8Array(53),r=new DataView(n.buffer);return r.setUint8(0,e.type),r.setUint32(1,e.mode,!0),r.setFloat64(5,e.size,!0),r.setFloat64(13,e.mtime,!0),r.setFloat64(21,e.ctime,!0),r.setFloat64(29,e.atime,!0),r.setUint32(37,e.uid,!0),r.setUint32(41,e.gid,!0),r.setUint32(45,t,!0),r.setUint32(49,s,!0),{status:0,data:n}}mkdir(t,e=0,s=511){if(t=this.normalizePath(t),(e&1)!==0)return this.mkdirRecursive(t,s);if(this.pathIndex.has(t)||this.isImplicitDirectory(t))return{status:y.EEXIST,data:null};const r=this.ensureParent(t);return r!==0?{status:r,data:null}:(this.createInode(t,p.DIRECTORY,this.dirModeFor(s),0),this.commitPending(),{status:0,data:null})}dirModeFor(t){return Le|t&4095&~(this.umask&511)}fileModeFor(t){return Pe|t&4095&~(this.umask&511)}mkdirRecursive(t,e=511){const s=t.split("/").filter(Boolean);let n="",r=null;for(const o of s){if(n+="/"+o,this.pathIndex.has(n)){const l=this.pathIndex.get(n);if(this.readInode(l).type!==p.DIRECTORY)return{status:y.ENOTDIR,data:null};continue}this.createInode(n,p.DIRECTORY,this.dirModeFor(e),0),r||(r=n)}return this.commitPending(),{status:0,data:(r?V.encode(r):void 0)??null}}rmdir(t,e=0){t=this.normalizePath(t);const s=(e&1)!==0,n=this.pathIndex.get(t);if(n===void 0){if(this.isImplicitDirectory(t)){if(this.getDirectChildrenWithImplicit(t).length>0){if(!s)return{status:y.ENOTEMPTY};for(const l of this.getAllDescendants(t)){const f=this.pathIndex.get(l),h=this.readInode(f);this.freeBlockRange(h.firstBlock,h.blockCount),h.type=p.FREE,this.writeInode(f,h),this.deletePathIndex(l)}this.pathIndexGen++,this.commitPending()}return{status:0}}return{status:y.ENOENT}}const r=this.readInode(n);if(r.type!==p.DIRECTORY)return{status:y.ENOTDIR};if(this.getDirectChildren(t).length>0){if(!s)return{status:y.ENOTEMPTY};for(const o of this.getAllDescendants(t)){const l=this.pathIndex.get(o),f=this.readInode(l);this.freeBlockRange(f.firstBlock,f.blockCount),f.type=p.FREE,this.writeInode(l,f),this.deletePathIndex(o)}}return t==="/"?(this.pathIndexGen++,this.commitPending(),{status:0}):(r.type=p.FREE,this.writeInode(n,r),this.deletePathIndex(t),this.pathIndexGen++,n<this.freeInodeHint&&(this.freeInodeHint=n),this.commitPending(),{status:0})}readdir(t,e=0){t=this.normalizePath(t);const s=this.resolvePathFull(t,!0);let n;if(s){if(this.readInode(s.idx).type!==p.DIRECTORY)return{status:y.ENOTDIR,data:null};n=s.resolvedPath}else if(this.isImplicitDirectory(t))n=t;else return{status:y.ENOENT,data:null};if((e&1)!==0){this.ensureChildIndex();const m=this.childIndex.get(n);if(!m)return{status:0,data:new Uint8Array([0,0,0,0])};const w=[...m.keys()].sort(),u=n==="/"?"/":n+"/";let E=4;for(const R of w)E+=3+R.length*3;const D=new Uint8Array(E),C=new DataView(D.buffer);C.setUint32(0,w.length,!0);let F=4;for(const R of w){const{written:$}=V.encodeInto(R,D.subarray(F+2));C.setUint16(F,$,!0),F+=2+$;const Qt=this.pathIndex.get(u+R);D[F++]=Qt===void 0?p.DIRECTORY:this.readInode(Qt).type}return{status:0,data:D.subarray(0,F)}}this.ensureChildIndex();const a=this.childIndex.get(n);if(!a)return{status:0,data:new Uint8Array([0,0,0,0])};const o=[...a.keys()].sort();let l=4;for(const m of o)l+=2+m.length*3;const f=new Uint8Array(l),h=new DataView(f.buffer);h.setUint32(0,o.length,!0);let d=4;for(const m of o){const{written:w}=V.encodeInto(m,f.subarray(d+2));h.setUint16(d,w,!0),d+=2+w}return{status:0,data:f.subarray(0,d)}}rename(t,e){t=this.normalizePath(t),e=this.normalizePath(e);const s=this.pathIndex.get(t);if(s===void 0)return{status:y.ENOENT};if(t===e)return{status:0};const n=this.ensureParent(e);if(n!==0)return{status:n};const r=this.pathIndex.get(e),a=r===void 0&&this.isImplicitDirectory(e);if(r!==void 0||a){const h=this.readInode(s).type===p.DIRECTORY,d=a||r!==void 0&&this.readInode(r).type===p.DIRECTORY;if(h&&!d)return{status:y.ENOTDIR};if(!h&&d)return{status:y.EISDIR}}if(r!==void 0||a){let h=a;if(r!==void 0){const d=this.readInode(r);h=d.type===p.DIRECTORY,this.freeBlockRange(d.firstBlock,d.blockCount),d.type=p.FREE,this.writeInode(r,d),this.deletePathIndex(e),r<this.freeInodeHint&&(this.freeInodeHint=r)}if(h)for(const d of this.getAllDescendants(e)){const m=this.pathIndex.get(d),w=this.readInode(m);this.freeBlockRange(w.firstBlock,w.blockCount),w.type=p.FREE,this.writeInode(m,w),this.deletePathIndex(d),m<this.freeInodeHint&&(this.freeInodeHint=m)}}const o=this.readInode(s),{offset:l,length:f}=this.appendPath(e);if(o.pathOffset=l,o.pathLength=f,o.mtime=Date.now(),this.writeInode(s,o),this.deletePathIndex(t),this.setPathIndex(e,s),this.pathIndexGen++,o.type===p.DIRECTORY){const h=t==="/"?"/":t+"/",d=[];for(const[m,w]of this.pathIndex)m.startsWith(h)&&d.push([m,w]);for(const[m,w]of d){const u=m.substring(t.length),E=e+u,D=this.readInode(w),{offset:C,length:F}=this.appendPath(E);D.pathOffset=C,D.pathLength=F,this.writeInode(w,D),this.deletePathIndex(m),this.setPathIndex(E,w)}}return this.commitPending(),{status:0}}exists(t){t=this.normalizePath(t);const e=this.resolvePathComponents(t,!0),s=new Uint8Array(1);return s[0]=e!==void 0||this.isImplicitDirectory(t)?1:0,{status:0,data:s}}truncate(t,e=0){t=this.normalizePath(t);const s=this.resolvePathComponents(t,!0);if(s===void 0)return{status:this.resolveFailureStatus()};const n=this.readInode(s);if(n.type===p.DIRECTORY)return{status:y.EISDIR};if(e===0)this.freeBlockRange(n.firstBlock,n.blockCount),n.firstBlock=0,n.blockCount=0,n.size=0;else if(e<n.size){const r=Math.ceil(e/this.blockSize);r<n.blockCount&&this.freeBlockRange(n.firstBlock+r,n.blockCount-r),n.blockCount=r,n.size=e}else if(e>n.size){const r=Math.ceil(e/this.blockSize);if(r>n.blockCount){const a=this.handle.getSize(),o=this.allocateBlocks(r),l=this.dataOffset+o*this.blockSize;if(n.size>0){const f=this.dataOffset+n.firstBlock*this.blockSize,h=4*1024*1024,d=new Uint8Array(Math.min(h,n.size));let m=0;for(;m<n.size;){const w=Math.min(h,n.size-m),u=w<d.length?d.subarray(0,w):d;this.handle.read(u,{at:f+m}),this.handle.write(u,{at:l+m}),m+=w}}this.freeBlockRange(n.firstBlock,n.blockCount),this.zeroFileRange(l+n.size,e-n.size,a),n.firstBlock=o}else this.zeroFileRange(this.dataOffset+n.firstBlock*this.blockSize+n.size,e-n.size);n.blockCount=r,n.size=e}return n.mtime=Date.now(),this.writeInode(s,n),this.commitPending(),{status:0}}copy(t,e,s=0){t=this.normalizePath(t),e=this.normalizePath(e);const n=this.resolvePathComponents(t,!0);if(n===void 0)return{status:this.resolveFailureStatus()};const r=this.readInode(n);if(r.type===p.DIRECTORY)return{status:y.ENOTSUP};if(s&1&&(this.pathIndex.has(e)||this.isImplicitDirectory(e)))return{status:y.EEXIST};if(t===e)return{status:0};const a=r.size,o=r.firstBlock,l=r.mode,f=this.write(e,new Uint8Array(0));if(f.status!==0)return f;if(a===0){const R=this.resolvePathComponents(e,!0);if(R!==void 0){const $=this.readInode(R);$.mode=$.mode&-4096|l&4095,this.writeInode(R,$),this.commitPending()}return{status:0}}const h=this.resolvePathComponents(e,!0);if(h===void 0)return{status:y.EIO};const d=this.readInode(h),m=Math.ceil(a/this.blockSize),w=this.allocateBlocks(m),u=this.dataOffset+w*this.blockSize,E=this.dataOffset+o*this.blockSize,D=4*1024*1024,C=new Uint8Array(Math.min(D,a));let F=0;for(;F<a;){const R=Math.min(D,a-F),$=R<C.length?C.subarray(0,R):C;this.handle.read($,{at:E+F}),this.handle.write($,{at:u+F}),F+=R}return d.firstBlock=w,d.blockCount=m,d.size=a,d.mtime=Date.now(),d.mode=d.mode&-4096|l&4095,this.writeInode(h,d),this.commitPending(),{status:0}}access(t,e=0){t=this.normalizePath(t);const s=this.resolvePathComponents(t,!0);if(s===void 0){const a=this.resolveFailureStatus();return this.isImplicitDirectory(t)?{status:0}:{status:a}}if(e===0)return{status:0};if(!this.strictPermissions)return{status:0};const n=this.readInode(s),r=this.getEffectivePermission(n);return e&4&&!(r&4)?{status:y.EACCES}:e&2&&!(r&2)?{status:y.EACCES}:e&1&&!(r&1)?{status:y.EACCES}:{status:0}}getEffectivePermission(t){const e=t.mode&511;return this.processUid===t.uid?e>>>6&7:this.processGid===t.gid?e>>>3&7:e&7}realpath(t){t=this.normalizePath(t);const e=this.resolvePathComponents(t,!0);if(e===void 0){const r=this.resolveFailureStatus();return this.isImplicitDirectory(t)?{status:0,data:V.encode(t)}:{status:r,data:null}}const s=this.readInode(e),n=this.readPath(s.pathOffset,s.pathLength);return{status:0,data:V.encode(n)}}chmod(t,e,s=!0){t=this.normalizePath(t);const n=this.resolvePathComponents(t,s);if(n===void 0)return{status:this.resolveFailureStatus()};const r=this.readInode(n);return r.mode=r.mode&Jt|e&4095,r.ctime=Date.now(),this.writeInode(n,r),{status:0}}chown(t,e,s,n=!0){t=this.normalizePath(t);const r=this.resolvePathComponents(t,n);if(r===void 0)return{status:this.resolveFailureStatus()};const a=this.readInode(r);return a.uid=e,a.gid=s,a.ctime=Date.now(),this.writeInode(r,a),{status:0}}utimes(t,e,s,n=!0){t=this.normalizePath(t);const r=this.resolvePathComponents(t,n);if(r===void 0)return{status:this.resolveFailureStatus()};const a=this.readInode(r);return a.atime=e,a.mtime=s,a.ctime=Date.now(),this.writeInode(r,a),{status:0}}symlink(t,e){if(e=this.normalizePath(e),this.pathIndex.has(e)||this.isImplicitDirectory(e))return{status:y.EEXIST};const s=this.ensureParent(e);if(s!==0)return{status:s};const n=V.encode(t);return this.createInode(e,p.SYMLINK,xe,n.byteLength,n),this.commitPending(),{status:0}}readlink(t){t=this.normalizePath(t);const e=this.pathIndex.get(t);if(e===void 0)return{status:y.ENOENT,data:null};const s=this.readInode(e);return s.type!==p.SYMLINK?{status:y.EINVAL,data:null}:{status:0,data:this.readData(s.firstBlock,s.blockCount,s.size)}}link(t,e){t=this.normalizePath(t),e=this.normalizePath(e);const s=this.resolvePathComponents(t,!0);if(s===void 0)return{status:this.resolveFailureStatus()};const n=this.readInode(s);if(n.type===p.DIRECTORY)return{status:y.EPERM};if(this.pathIndex.has(e)||this.isImplicitDirectory(e))return{status:y.EEXIST};const r=this.copy(t,e);if(r.status!==0)return r;n.nlink++,this.writeInode(s,n);const a=this.pathIndex.get(e);if(a!==void 0){const o=this.readInode(a);o.nlink=n.nlink,this.writeInode(a,o)}return{status:0}}open(t,e,s,n=438){t=this.normalizePath(t);const r=(e&64)!==0,a=(e&512)!==0,o=(e&128)!==0;let l=this.resolvePathComponents(t,!0);if(l===void 0){const d=this.resolveDanglingLink(t);if(d===null)return{status:y.ELOOP,data:null};d!==t&&(t=d,l=this.resolvePathComponents(t,!0))}if(Tt.isWritable(e)&&(l!==void 0?this.readInode(l).type===p.DIRECTORY:this.isImplicitDirectory(t)))return{status:y.EISDIR,data:null};if(l===void 0){if(!r)return{status:this.resolveFailureStatus(),data:null};const d=this.ensureParent(t);if(d!==0)return{status:d,data:null};l=this.createInode(t,p.FILE,this.fileModeFor(n),0)}else if(o&&r)return{status:y.EEXIST,data:null};a&&this.truncate(t,0);const f=this.nextFd++;this.fdTable.set(f,{tabId:s,inodeIdx:l,position:0,flags:e});const h=new Uint8Array(4);return new DataView(h.buffer).setUint32(0,f,!0),{status:0,data:h}}close(t){return this.fdTable.has(t)?(this.fdTable.delete(t),{status:0}):{status:y.EBADF}}fread(t,e,s){const n=this.fdTable.get(t);if(!n)return{status:y.EBADF,data:null};if(!Tt.isReadable(n.flags))return{status:y.EBADF,data:null};const r=this.readInode(n.inodeIdx);if(r.type===p.DIRECTORY)return{status:y.EISDIR,data:null};const a=s??n.position,o=Math.min(e,r.size-a);if(o<=0)return{status:0,data:new Uint8Array(0)};const l=this.dataOffset+r.firstBlock*this.blockSize+a,f=new Uint8Array(o);return this.handle.read(f,{at:l}),s===null&&(n.position+=o),{status:0,data:f}}fwrite(t,e,s){const n=this.fdTable.get(t);if(!n)return{status:y.EBADF,data:null};if(!Tt.isWritable(n.flags))return{status:y.EBADF,data:null};const r=this.readInode(n.inodeIdx),o=(n.flags&1024)!==0?r.size:s??n.position,l=o+e.byteLength;if(l>r.size){const h=Math.ceil(l/this.blockSize);if(h>r.blockCount){const d=this.handle.getSize(),m=this.allocateBlocks(h),w=this.dataOffset+m*this.blockSize,u=this.dataOffset+r.firstBlock*this.blockSize;if(r.size>0){const D=new Uint8Array(Math.min(4194304,r.size));let C=0;for(;C<r.size;){const F=Math.min(4194304,r.size-C),R=F<D.length?D.subarray(0,F):D;this.handle.read(R,{at:u+C}),this.handle.write(R,{at:w+C}),C+=F}}this.freeBlockRange(r.firstBlock,r.blockCount),o>r.size&&this.zeroFileRange(w+r.size,o-r.size,d),this.handle.write(e,{at:w+o}),r.firstBlock=m,r.blockCount=h}else{o>r.size&&this.zeroFileRange(this.dataOffset+r.firstBlock*this.blockSize+r.size,o-r.size);const d=this.dataOffset+r.firstBlock*this.blockSize+o;this.handle.write(e,{at:d})}r.size=l}else{const h=this.dataOffset+r.firstBlock*this.blockSize+o;this.handle.write(e,{at:h})}r.mtime=Date.now(),this.writeInode(n.inodeIdx,r),s===null&&(n.position=l),this.commitPending();const f=new Uint8Array(4);return new DataView(f.buffer).setUint32(0,e.byteLength,!0),{status:0,data:f}}fstat(t){const e=this.fdTable.get(t);return e?e.implicitPath?this.encodeImplicitDirStatResponse(e.implicitPath):this.encodeStatResponse(e.inodeIdx):{status:y.EBADF,data:null}}ftruncate(t,e=0){const s=this.fdTable.get(t);if(!s)return{status:y.EBADF};if(!Tt.isWritable(s.flags))return{status:y.EINVAL};const n=this.readInode(s.inodeIdx),r=this.readPath(n.pathOffset,n.pathLength);return this.truncate(r,e)}statfs(t="/"){if(t=this.normalizePath(t),this.resolvePathComponents(t,!0)===void 0&&!this.isImplicitDirectory(t))return{status:this.resolveFailureStatus(),data:null};const e=new Set(this.pathIndex.values()).size,s=new Uint8Array(24),n=new DataView(s.buffer);return n.setUint32(0,lt,!0),n.setUint32(4,this.blockSize,!0),n.setUint32(8,this.totalBlocks,!0),n.setUint32(12,this.freeBlocks,!0),n.setUint32(16,this.inodeCount,!0),n.setUint32(20,Math.max(0,this.inodeCount-e),!0),{status:0,data:s}}fsync(t){return t!==void 0&&!this.fdTable.has(t)?{status:y.EBADF}:(this.commitPending(),this.handle.flush(),{status:0})}fchmod(t,e){const s=this.fdTable.get(t);if(!s)return{status:y.EBADF};if(s.implicitPath)return{status:0};const n=this.readInode(s.inodeIdx);return n.mode=n.mode&Jt|e&4095,n.ctime=Date.now(),this.writeInode(s.inodeIdx,n),{status:0}}fchown(t,e,s){const n=this.fdTable.get(t);if(!n)return{status:y.EBADF};if(n.implicitPath)return{status:0};const r=this.readInode(n.inodeIdx);return r.uid=e,r.gid=s,r.ctime=Date.now(),this.writeInode(n.inodeIdx,r),{status:0}}futimes(t,e,s){const n=this.fdTable.get(t);if(!n)return{status:y.EBADF};if(n.implicitPath)return{status:0};const r=this.readInode(n.inodeIdx);return r.atime=e,r.mtime=s,r.ctime=Date.now(),this.writeInode(n.inodeIdx,r),{status:0}}opendir(t,e){t=this.normalizePath(t);const s=this.resolvePathComponents(t,!0);if(s===void 0){if(this.isImplicitDirectory(t)){const o=this.nextFd++;this.fdTable.set(o,{tabId:e,inodeIdx:-1,position:0,flags:0,implicitPath:t});const l=new Uint8Array(4);return new DataView(l.buffer).setUint32(0,o,!0),{status:0,data:l}}return{status:y.ENOENT,data:null}}if(this.readInode(s).type!==p.DIRECTORY)return{status:y.ENOTDIR,data:null};const r=this.nextFd++;this.fdTable.set(r,{tabId:e,inodeIdx:s,position:0,flags:0});const a=new Uint8Array(4);return new DataView(a.buffer).setUint32(0,r,!0),{status:0,data:a}}mkdtemp(t){const e=Math.random().toString(36).substring(2,8),s=this.normalizePath(t+e),n=s.substring(0,s.lastIndexOf("/"))||"/";return n!=="/"&&this.resolvePathComponents(n,!0)===void 0&&!this.isImplicitDirectory(n)?{status:y.ENOENT,data:null}:(this.createInode(s,p.DIRECTORY,this.dirModeFor(448),0),this.commitPending(),{status:0,data:V.encode(s)})}getDirectChildren(t){this.ensureChildIndex();const e=this.childIndex.get(t);if(!e)return[];const s=t==="/"?"/":t+"/",n=[];for(const r of e.keys()){const a=s+r;this.pathIndex.has(a)&&n.push(a)}return n.sort()}rebuildImplicitDirs(){if(this.implicitDirsGen===this.pathIndexGen)return;const t=Date.now(),e=this.implicitDirs;this.implicitDirs=new Map;for(const s of this.pathIndex.keys()){let n=s.length;for(;n=s.lastIndexOf("/",n-1),!(n<=0);){const r=s.substring(0,n);if(this.implicitDirs.has(r))break;this.pathIndex.has(r)||this.implicitDirs.set(r,e.get(r)??t)}}this.implicitDirsGen=this.pathIndexGen}isImplicitDirectory(t){return t==="/"||this.pathIndex.has(t)?!1:(this.descCountGen<this.pathIndexGen&&this.rebuildDescCount(),(this.descCount.get(t)??0)>0)}rebuildDescCount(){this.descCount.clear();for(const t of this.pathIndex.keys())this.bumpDescCount(t);this.descCountGen=this.pathIndexGen}setPathIndex(t,e){const s=this.pathIndex.has(t);this.pathIndex.set(t,e),s||(this.bumpDescCount(t),this.bumpChildIndex(t)),this.descCountGen=this.pathIndexGen+1,this.childIndexGen=this.pathIndexGen+1}deletePathIndex(t){const e=this.pathIndex.delete(t);return e&&(this.decDescCount(t),this.decChildIndex(t)),this.descCountGen=this.pathIndexGen+1,this.childIndexGen=this.pathIndexGen+1,e}bumpDescCount(t){let e=t.length;for(;e=t.lastIndexOf("/",e-1),!(e<=0);){const s=t.substring(0,e);this.descCount.set(s,(this.descCount.get(s)??0)+1)}}decDescCount(t){let e=t.length;for(;e=t.lastIndexOf("/",e-1),!(e<=0);){const s=t.substring(0,e),n=this.descCount.get(s);if(n===void 0)break;n<=1?this.descCount.delete(s):this.descCount.set(s,n-1)}}bumpChildIndex(t){if(t==="/"||t.length===0)return;let e="/",s=1;for(;s<=t.length;){let n=t.indexOf("/",s);n===-1&&(n=t.length);const r=t.substring(s,n);if(r.length>0){let a=this.childIndex.get(e);a||(a=new Map,this.childIndex.set(e,a)),a.set(r,(a.get(r)??0)+1),e=e==="/"?"/"+r:e+"/"+r}s=n+1}}decChildIndex(t){if(t==="/"||t.length===0)return;let e="/",s=1;for(;s<=t.length;){let n=t.indexOf("/",s);n===-1&&(n=t.length);const r=t.substring(s,n);if(r.length>0){const a=this.childIndex.get(e);if(!a)break;const o=a.get(r);if(o===void 0)break;o<=1?(a.delete(r),a.size===0&&this.childIndex.delete(e)):a.set(r,o-1),e=e==="/"?"/"+r:e+"/"+r}s=n+1}}ensureChildIndex(){if(!(this.childIndexGen>=this.pathIndexGen)){this.childIndex.clear();for(const t of this.pathIndex.keys())this.bumpChildIndex(t);this.childIndexGen=this.pathIndexGen}}countSubdirectories(t){this.ensureChildIndex();const e=this.childIndex.get(t);if(!e)return 0;const s=t==="/"?"/":t+"/";let n=0;for(const r of e.keys()){const a=this.pathIndex.get(s+r);(a===void 0||this.readInode(a).type===p.DIRECTORY)&&n++}return n}getDirectChildrenWithImplicit(t){this.ensureChildIndex();const e=this.childIndex.get(t);if(!e)return[];const s=t==="/"?"/":t+"/",n=[];for(const r of e.keys()){const a=s+r;n.push({path:a,type:this.pathIndex.has(a)?"real":"implicit"})}return n.sort((r,a)=>r.path<a.path?-1:r.path>a.path?1:0),n}encodeImplicitDirStatResponse(t){this.rebuildImplicitDirs();const e=this.implicitDirs.get(t)??Date.now(),s=Xt&~(this.umask&511),n=2+this.countSubdirectories(t),r=new Uint8Array(53),a=new DataView(r.buffer);return a.setUint8(0,p.DIRECTORY),a.setUint32(1,s,!0),a.setFloat64(5,0,!0),a.setFloat64(13,e,!0),a.setFloat64(21,e,!0),a.setFloat64(29,e,!0),a.setUint32(37,this.processUid,!0),a.setUint32(41,this.processGid,!0),a.setUint32(45,0,!0),a.setUint32(49,n,!0),{status:0,data:r}}getAllDescendants(t){const e=t==="/"?"/":t+"/",s=[];for(const n of this.pathIndex.keys())n!==t&&n.startsWith(e)&&s.push(n);return s.sort((n,r)=>{const a=n.split("/").length;return r.split("/").length-a})}ensureParent(t){const e=t.lastIndexOf("/");if(e<=0)return 0;const s=t.substring(0,e),n=this.pathIndex.get(s);return n===void 0?this.isImplicitDirectory(s)?0:y.ENOENT:this.readInode(n).type!==p.DIRECTORY?y.ENOTDIR:0}cleanupTab(t){for(const[e,s]of this.fdTable)s.tabId===t&&this.fdTable.delete(e)}getAllFiles(){const t=[];for(const[e,s]of this.pathIndex)t.push({path:e,idx:s});return t}getPathForFd(t){const e=this.fdTable.get(t);if(!e)return null;const s=this.readInode(e.inodeIdx);return this.readPath(s.pathOffset,s.pathLength)}getInodeData(t){const e=this.readInode(t),s=e.size>0?this.readData(e.firstBlock,e.blockCount,e.size):new Uint8Array(0);return{type:e.type,data:s,mtime:e.mtime}}exportAll(){const t=[];for(const[e,s]of this.pathIndex){const n=this.readInode(s);let r=null;(n.type===p.FILE||n.type===p.SYMLINK)&&(r=n.size>0?this.readData(n.firstBlock,n.blockCount,n.size):new Uint8Array(0)),t.push({path:e,type:n.type,data:r,mode:n.mode,mtime:n.mtime})}return t.sort((e,s)=>e.type===p.DIRECTORY&&s.type!==p.DIRECTORY?-1:e.type!==p.DIRECTORY&&s.type===p.DIRECTORY?1:e.path.localeCompare(s.path)),t}flush(){this.handle.flush()}},kt=new TextEncoder,ze=1447449377,ne=1,ie=2,g=0,O=1,Bt=2,_e=5,zt=7,K=8,He=class{rootDir;fdTable=new Map;nextFd=3;nextIno=1;processUid=0;processGid=0;async init(i,t){this.rootDir=i,this.processUid=t?.uid??0,this.processGid=t?.gid??0}cleanupTab(i){for(const[t,e]of this.fdTable){try{e.handle.close()}catch{}this.fdTable.delete(t)}}getPathForFd(i){return this.fdTable.get(i)?.path??null}normalizePath(i){for(i.startsWith("/")||(i="/"+i);i.length>1&&i.endsWith("/");)i=i.slice(0,-1);const t=i.split("/"),e=[];for(const s of t)if(!(s===""||s===".")){if(s===".."){e.pop();continue}e.push(s)}return"/"+e.join("/")}async navigateToParent(i){const t=i.split("/").filter(Boolean);if(t.length===0)return null;const e=t.pop();let s=this.rootDir;for(const n of t)try{s=await s.getDirectoryHandle(n)}catch{return null}return{dir:s,name:e}}async navigateToDir(i){if(i==="/")return this.rootDir;const t=i.split("/").filter(Boolean);let e=this.rootDir;for(const s of t)try{e=await e.getDirectoryHandle(s)}catch{return null}return e}async getEntry(i){if(i==="/")return{handle:this.rootDir,kind:"directory"};const t=await this.navigateToParent(i);if(!t)return null;try{return{handle:await t.dir.getFileHandle(t.name),kind:"file"}}catch{try{return{handle:await t.dir.getDirectoryHandle(t.name),kind:"directory"}}catch{return null}}}async ensureParent(i){const t=i.split("/").filter(Boolean);t.pop();let e=this.rootDir;for(const s of t)try{e=await e.getDirectoryHandle(s,{create:!0})}catch{return null}return e}encodeStat(i,t,e,s){const n=new Uint8Array(53),r=new DataView(n.buffer);return r.setUint8(0,i==="file"?ne:ie),r.setUint32(1,i==="file"?33188:16877,!0),r.setFloat64(5,t,!0),r.setFloat64(13,e,!0),r.setFloat64(21,e,!0),r.setFloat64(29,e,!0),r.setUint32(37,this.processUid,!0),r.setUint32(41,this.processGid,!0),r.setUint32(45,s,!0),r.setUint32(49,i==="directory"?2:1,!0),n}async read(i){i=this.normalizePath(i);const t=await this.navigateToParent(i);if(!t)return{status:O,data:null};try{const s=await(await t.dir.getFileHandle(t.name)).createSyncAccessHandle();try{const n=s.getSize(),r=new Uint8Array(n);return n>0&&s.read(r,{at:0}),{status:g,data:r}}finally{s.close()}}catch{return{status:O,data:null}}}async write(i,t,e){i=this.normalizePath(i);const s=await this.ensureParent(i);if(!s)return{status:O,data:null};const n=i.split("/").filter(Boolean).pop();try{const a=await(await s.getFileHandle(n,{create:!0})).createSyncAccessHandle();try{a.truncate(0),t.byteLength>0&&a.write(t,{at:0}),a.flush()}finally{a.close()}return{status:g,data:null}}catch{return{status:O,data:null}}}async append(i,t){i=this.normalizePath(i);const e=await this.ensureParent(i);if(!e)return{status:O,data:null};const s=i.split("/").filter(Boolean).pop();try{const r=await(await e.getFileHandle(s,{create:!0})).createSyncAccessHandle();try{const a=r.getSize();r.write(t,{at:a}),r.flush()}finally{r.close()}return{status:g,data:null}}catch{return{status:O,data:null}}}async unlink(i){i=this.normalizePath(i);const t=await this.navigateToParent(i);if(!t)return{status:O,data:null};try{return await t.dir.getFileHandle(t.name),await t.dir.removeEntry(t.name),{status:g,data:null}}catch{return{status:O,data:null}}}async stat(i){i=this.normalizePath(i);const t=await this.getEntry(i);if(!t)return{status:O,data:null};if(t.kind==="file"){const e=await t.handle.getFile();return{status:g,data:this.encodeStat("file",e.size,e.lastModified,this.nextIno++)}}return{status:g,data:this.encodeStat("directory",0,Date.now(),this.nextIno++)}}async lstat(i){return this.stat(i)}async mkdir(i,t=0,e=511){if(i=this.normalizePath(i),(t&1)!==0){const r=i.split("/").filter(Boolean);let a=this.rootDir,o=null,l="";for(const f of r){l+="/"+f;let h=!0;try{a=await a.getDirectoryHandle(f)}catch{h=!1,a=await a.getDirectoryHandle(f,{create:!0})}!h&&!o&&(o=l)}return{status:g,data:o?kt.encode(o):null}}const n=await this.navigateToParent(i);if(!n)return{status:O,data:null};try{try{return await n.dir.getDirectoryHandle(n.name),{status:Bt,data:null}}catch{}return await n.dir.getDirectoryHandle(n.name,{create:!0}),{status:g,data:null}}catch{return{status:O,data:null}}}async rmdir(i,t=0){if(i=this.normalizePath(i),i==="/")return{status:zt,data:null};const e=(t&1)!==0,s=await this.navigateToParent(i);if(!s)return{status:O,data:null};try{return await s.dir.getDirectoryHandle(s.name),await s.dir.removeEntry(s.name,{recursive:e}),{status:g,data:null}}catch(n){return n.name==="InvalidModificationError"?{status:_e,data:null}:{status:O,data:null}}}async readdir(i,t=0){i=this.normalizePath(i);const e=await this.navigateToDir(i);if(!e)return{status:O,data:null};const s=(t&1)!==0,n=[];for await(const[h,d]of e.entries())n.push({name:h,kind:d.kind});if(s){let h=4;const d=[];for(const E of n){const D=kt.encode(E.name);d.push({nameBytes:D,type:E.kind==="file"?ne:ie}),h+=2+D.byteLength+1}const m=new Uint8Array(h),w=new DataView(m.buffer);w.setUint32(0,d.length,!0);let u=4;for(const E of d)w.setUint16(u,E.nameBytes.byteLength,!0),u+=2,m.set(E.nameBytes,u),u+=E.nameBytes.byteLength,m[u++]=E.type;return{status:g,data:m}}let r=4;const a=[];for(const h of n){const d=kt.encode(h.name);a.push(d),r+=2+d.byteLength}const o=new Uint8Array(r),l=new DataView(o.buffer);l.setUint32(0,a.length,!0);let f=4;for(const h of a)l.setUint16(f,h.byteLength,!0),f+=2,o.set(h,f),f+=h.byteLength;return{status:g,data:o}}async rename(i,t){if(i=this.normalizePath(i),t=this.normalizePath(t),i===t)return{status:g,data:null};const e=await this.getEntry(i);if(!e)return{status:O,data:null};if(e.kind==="file"){const n=await e.handle.getFile(),r=new Uint8Array(await n.arrayBuffer()),a=await this.write(t,r);if(a.status!==g)return a;const o=await this.removeSourceWithRetry(()=>this.unlink(i));if(o.status!==g)return o}else{const s=await this.getEntry(t);if(s){const a=s.kind==="directory"?await this.rmdir(t,1):await this.unlink(t);if(a.status!==g)return a}const n=await this.mkdir(t,1);if(n.status!==g&&n.status!==Bt)return n;await this.copyDirectoryContents(i,t);const r=await this.removeSourceWithRetry(()=>this.rmdir(i,1));if(r.status!==g)return r}return{status:g,data:null}}async removeSourceWithRetry(i){let t=await i();for(let e=0;e<3&&t.status!==g;e++)await new Promise(s=>setTimeout(s,10*(e+1))),t=await i();return t}async copyDirectoryContents(i,t){const e=await this.navigateToDir(i);if(e)for await(const[s,n]of e.entries()){const r=i==="/"?`/${s}`:`${i}/${s}`,a=t==="/"?`/${s}`:`${t}/${s}`;if(n.kind==="directory")await this.mkdir(a,1),await this.copyDirectoryContents(r,a);else{const o=await n.getFile(),l=new Uint8Array(await o.arrayBuffer());await this.write(a,l)}}}async exists(i){i=this.normalizePath(i);const t=await this.getEntry(i);return{status:g,data:new Uint8Array([t?1:0])}}async truncate(i,t){i=this.normalizePath(i);const e=await this.navigateToParent(i);if(!e)return{status:O,data:null};try{const n=await(await e.dir.getFileHandle(e.name)).createSyncAccessHandle();try{n.truncate(t),n.flush()}finally{n.close()}return{status:g,data:null}}catch{return{status:O,data:null}}}async copy(i,t,e){i=this.normalizePath(i),t=this.normalizePath(t);const s=await this.read(i);return s.status!==g?s:this.write(t,s.data??new Uint8Array(0))}async access(i,t){return i=this.normalizePath(i),await this.getEntry(i)?{status:g,data:null}:{status:O,data:null}}async realpath(i){return i=this.normalizePath(i),await this.getEntry(i)?{status:g,data:kt.encode(i)}:{status:O,data:null}}async chmod(i,t){return i=this.normalizePath(i),await this.getEntry(i)?{status:g,data:null}:{status:O,data:null}}async chown(i,t,e){return i=this.normalizePath(i),await this.getEntry(i)?{status:g,data:null}:{status:O,data:null}}async utimes(i,t,e){return i=this.normalizePath(i),await this.getEntry(i)?{status:g,data:null}:{status:O,data:null}}async fchmod(i,t){return this.fdTable.has(i)?{status:g,data:null}:{status:K,data:null}}async fchown(i,t,e){return this.fdTable.has(i)?{status:g,data:null}:{status:K,data:null}}async futimes(i,t,e){return this.fdTable.has(i)?{status:g,data:null}:{status:K,data:null}}async symlink(i,t){return{status:zt,data:null}}async readlink(i){return{status:zt,data:null}}async link(i,t){return this.copy(i,t)}async open(i,t,e,s=438){i=this.normalizePath(i);const n=(t&64)!==0,r=(t&512)!==0,a=(t&128)!==0,o=await this.ensureParent(i);if(!o)return{status:O,data:null};const l=i.split("/").filter(Boolean).pop();try{let f=!0;try{await o.getFileHandle(l)}catch{f=!1}if(!f&&!n)return{status:O,data:null};if(f&&a&&n)return{status:Bt,data:null};const d=await(await o.getFileHandle(l,{create:n})).createSyncAccessHandle();r&&(d.truncate(0),d.flush());const m=this.nextFd++;this.fdTable.set(m,{handle:d,path:i,position:0,flags:t});const w=new Uint8Array(4);return new DataView(w.buffer).setUint32(0,m,!0),{status:g,data:w}}catch{return{status:O,data:null}}}async close(i){const t=this.fdTable.get(i);if(!t)return{status:K,data:null};try{t.handle.close()}catch{}return this.fdTable.delete(i),{status:g,data:null}}async fread(i,t,e){const s=this.fdTable.get(i);if(!s)return{status:K,data:null};const n=e??s.position,r=s.handle.getSize(),a=Math.min(t,r-n);if(a<=0)return{status:g,data:new Uint8Array(0)};const o=new Uint8Array(a);return s.handle.read(o,{at:n}),e===null&&(s.position+=a),{status:g,data:o}}async fwrite(i,t,e){const s=this.fdTable.get(i);if(!s)return{status:K,data:null};const r=(s.flags&1024)!==0?s.handle.getSize():e??s.position;s.handle.write(t,{at:r}),e===null&&(s.position=r+t.byteLength);const a=new Uint8Array(4);return new DataView(a.buffer).setUint32(0,t.byteLength,!0),{status:g,data:a}}async fstat(i){const t=this.fdTable.get(i);if(!t)return{status:K,data:null};const e=t.handle.getSize();return{status:g,data:this.encodeStat("file",e,Date.now(),i)}}async ftruncate(i,t=0){const e=this.fdTable.get(i);return e?(e.handle.truncate(t),e.handle.flush(),{status:g,data:null}):{status:K,data:null}}async statfs(){let t=0,e=0;try{const o=await navigator.storage.estimate();t=o.quota??0,e=o.usage??0}catch{}const s=Math.floor(t/4096),n=Math.max(0,Math.floor((t-e)/4096)),r=new Uint8Array(24),a=new DataView(r.buffer);return a.setUint32(0,ze,!0),a.setUint32(4,4096,!0),a.setUint32(8,Math.min(s,4294967295),!0),a.setUint32(12,Math.min(n,4294967295),!0),a.setUint32(16,0,!0),a.setUint32(20,0,!0),{status:0,data:r}}async fsync(i){if(i!==void 0&&!this.fdTable.has(i))return{status:K,data:null};for(const[,t]of this.fdTable)try{t.handle.flush()}catch{}return{status:g,data:null}}async opendir(i,t){return this.readdir(i,1)}async mkdtemp(i){const t=Math.random().toString(36).substring(2,8),e=this.normalizePath(i+t);return this.mkdir(e,1)}},c={READ:1,WRITE:2,UNLINK:3,STAT:4,LSTAT:5,MKDIR:6,RMDIR:7,READDIR:8,RENAME:9,EXISTS:10,TRUNCATE:11,APPEND:12,COPY:13,ACCESS:14,REALPATH:15,CHMOD:16,CHOWN:17,UTIMES:18,SYMLINK:19,READLINK:20,LINK:21,OPEN:22,CLOSE:23,FREAD:24,FWRITE:25,FSTAT:26,FTRUNCATE:27,FSYNC:28,OPENDIR:29,MKDTEMP:30,FCHMOD:31,FCHOWN:32,FUTIMES:33,STATFS:34},Y={OK:0,ENOENT:1,EEXIST:2,EISDIR:3,ENOTDIR:4,ENOTEMPTY:5,EACCES:6,EINVAL:7,EBADF:8,ELOOP:9,ENOSPC:10,EIO:11},j={CONTROL:0,TICKET_NEXT:4,TICKET_SERVING:8,OPCODE:4,STATUS:8,CHUNK_LEN:12,TOTAL_LEN:16,CHUNK_IDX:24,HEARTBEAT:28,HEADER_SIZE:32},S={IDLE:0,REQUEST:1,RESPONSE:2,CHUNK:3,CHUNK_ACK:4},Ps=new TextEncoder,re=new TextDecoder;function ae(i){if(i.byteLength<16)throw new Error(`Request buffer too small: ${i.byteLength} < 16 bytes (possible SAB race)`);const t=new DataView(i),e=t.getUint32(0,!0),s=t.getUint32(4,!0),n=t.getUint32(8,!0),r=t.getUint32(12,!0),a=16+n+r;if(i.byteLength<a)throw new Error(`Request buffer truncated: ${i.byteLength} < ${a} bytes (op=${e}, pathLen=${n}, dataLen=${r})`);const o=new Uint8Array(i),l=re.decode(o.subarray(16,16+n)),f=r>0?o.subarray(16+n,16+n+r):null;return{op:e,flags:s,path:l,data:f}}function W(i,t){const e=t?t.byteLength:0,s=new ArrayBuffer(8+e),n=new DataView(s);return n.setUint32(0,i,!0),n.setUint32(4,e,!0),t&&new Uint8Array(s).set(t,8),s}function G(i){const e=new DataView(i.buffer,i.byteOffset,i.byteLength).getUint32(0,!0);return re.decode(i.subarray(4,4+e))}var $e=1e4,Ke=1e3,Ve=class ve{constructor(t,e=$e,s){this.getTabId=t,this.deadlineMs=e,this.onUnprovenTimeout=s}port=null;pendingResolve=null;pendingSeq=0;pendingAbandoned=!1;seqCounter=0;timer=null;portProven=!1;get hasPort(){return this.port!==null}setPort(t){this.port&&this.port!==t&&(this.port.close(),this.abortPending()),this.port=t,this.portProven=!1}postRaw(t,e){this.port?.postMessage(t,e)}portSuspectUntil=0;static SUSPECT_WINDOW_MS=3e4;get portSuspect(){return Date.now()<this.portSuspectUntil}forward(t,e=!1){return e&&this.portSuspect?Promise.resolve(W(Y.EIO)):(this.pendingResolve&&this.abortPending(),new Promise(s=>{const n=++this.seqCounter;this.pendingSeq=n,this.pendingAbandoned=!1,this.pendingResolve=s;const r=t.buffer.byteLength===t.byteLength?t.buffer:t.slice().buffer;this.port.postMessage({id:n,tabId:this.getTabId(),buffer:r},[r]);const a=this.portProven?this.deadlineMs:Ke;this.timer=setTimeout(()=>{if(this.timer=null,this.pendingSeq===n&&this.pendingResolve){const o=this.pendingResolve;this.pendingResolve=null,this.pendingAbandoned=!0;const l=!this.portProven;l||(this.portSuspectUntil=Date.now()+ve.SUSPECT_WINDOW_MS),o(W(Y.EIO)),l&&this.onUnprovenTimeout?.()}},a)}))}handleResponse(t,e){if(this.portSuspectUntil=0,this.portProven=!0,t===this.pendingSeq){if(this.pendingResolve){this.clearTimer();const s=this.pendingResolve;return this.pendingResolve=null,s(e),!0}if(this.pendingAbandoned)return this.pendingAbandoned=!1,!0}return!1}abortPending(){if(this.clearTimer(),this.pendingResolve){const t=this.pendingResolve;this.pendingResolve=null,this.pendingAbandoned=!1,t(W(Y.EIO))}}clearTimer(){this.timer!==null&&(clearTimeout(this.timer),this.timer=null)}},oe=12e4,Ye=1e3,We=class extends Error{constructor(i){super(`SAB protocol wait timed out: ${i}`),this.name="SabWaitTimeoutError"}};function ce(i,t,e,s,n=Ye){for(;Atomics.load(i,0)===t;){const r=e-Date.now();if(r<=0)throw new We(s);Atomics.wait(i,0,t,Math.min(n,r))}}function Ge(i,t,e,s){try{const n=i.read(e);if(n.status===0){const r=[],a=[];if(n.data&&n.data.byteLength>0){const o=n.data.buffer.byteLength===n.data.byteLength?n.data.buffer:n.data.slice().buffer;r.push({op:"write",path:e,data:o,ts:s}),a.push(o)}else r.push({op:"write",path:e,data:new ArrayBuffer(0),ts:s});return r.push({op:"delete",path:t,ts:s}),{messages:r,transfers:a}}}catch{}return{messages:[{op:"rename",path:t,newPath:e,ts:s}],transfers:[]}}function Ze(i,t,e){const s=t.endsWith("/")?t:t+"/",n=e.endsWith("/")?e:e+"/",r=[];for(const a of i)a.startsWith(s)&&r.push({from:a,to:n+a.slice(s.length)});return r}function le(i){if(i.startsWith("/")||(i="/"+i),i.length===1)return i;const t=[];for(const e of i.split("/"))if(!(e===""||e===".")){if(e===".."){t.pop();continue}t.push(e)}return"/"+t.join("/")}function qe(i,t){if(t.startsWith("/"))return le(t);const e=i.lastIndexOf("/"),s=e<=0?"/":i.slice(0,e);return le(s+"/"+t)}function Qe(i,t,e,s){Ot(i,t,e),t.set(e,s);let n=i.get(s);n||(n=new Set,i.set(s,n)),n.add(e)}function Ot(i,t,e){const s=t.get(e);if(s===void 0)return;t.delete(e);const n=i.get(s);n&&(n.delete(e),n.size===0&&i.delete(s))}function _t(i,t){const e=t.endsWith("/")?t:t+"/",s=[];for(const n of i)(n===t||n.startsWith(e))&&s.push(n);return s}var L=i=>new DataView(i.buffer,i.byteOffset,i.byteLength),M=(i,t)=>!i||i.byteLength<t;function Ht(i){return M(i,4)?null:L(i).getUint32(0,!0)}function ue(i){return M(i,8)?null:L(i).getFloat64(0,!0)}function de(i){if(M(i,8))return null;const t=L(i);return{uid:t.getUint32(0,!0),gid:t.getUint32(4,!0)}}function fe(i){if(M(i,16))return null;const t=L(i);return{atime:t.getFloat64(0,!0),mtime:t.getFloat64(8,!0)}}function J(i){return M(i,4)?null:L(i).getUint32(0,!0)}function he(i){if(M(i,16))return null;const t=L(i),e=t.getFloat64(8,!0);return{fd:t.getUint32(0,!0),length:t.getUint32(4,!0),position:e===-1?null:e}}function me(i){if(M(i,12))return null;const t=L(i),e=t.getFloat64(4,!0);return{fd:t.getUint32(0,!0),position:e===-1?null:e,bytes:i.subarray(12)}}function we(i){if(M(i,12))return null;const t=L(i);return{fd:t.getUint32(0,!0),len:t.getFloat64(4,!0)}}function ye(i){if(M(i,8))return null;const t=L(i);return{fd:t.getUint32(0,!0),mode:t.getUint32(4,!0)}}function pe(i){if(M(i,12))return null;const t=L(i);return{fd:t.getUint32(0,!0),uid:t.getUint32(4,!0),gid:t.getUint32(8,!0)}}function be(i){if(M(i,24))return null;const t=L(i);return{fd:t.getUint32(0,!0),atime:t.getFloat64(8,!0),mtime:t.getFloat64(16,!0)}}var Xe=511,je=438,U=y.EINVAL,$t=1;function Ie(i,t){return Ht(i??null)??t}function Je(i,t,e,s,n,r){switch(e){case c.READ:return i.read(n);case c.WRITE:return i.write(n,r??new Uint8Array(0),s);case c.APPEND:return i.append(n,r??new Uint8Array(0));case c.UNLINK:return i.unlink(n);case c.STAT:return i.stat(n);case c.LSTAT:return i.lstat(n);case c.MKDIR:return i.mkdir(n,s,Ie(r,Xe));case c.RMDIR:return i.rmdir(n,s);case c.READDIR:return i.readdir(n,s);case c.RENAME:return i.rename(n,r?G(r):"");case c.EXISTS:return i.exists(n);case c.COPY:return i.copy(n,r?G(r):"",s);case c.ACCESS:return i.access(n,s);case c.REALPATH:return i.realpath(n);case c.READLINK:return i.readlink(n);case c.LINK:return i.link(n,r?G(r):"");case c.OPENDIR:return i.opendir(n,t);case c.MKDTEMP:return i.mkdtemp(n);case c.FSYNC:return i.fsync(J(r)??void 0);case c.STATFS:return i.statfs(n);case c.TRUNCATE:{const a=ue(r);return a===null?{status:U}:i.truncate(n,a)}case c.CHMOD:{const a=Ht(r);return a===null?{status:U}:i.chmod(n,a,(s&$t)===0)}case c.CHOWN:{const a=de(r);return a===null?{status:U}:i.chown(n,a.uid,a.gid,(s&$t)===0)}case c.UTIMES:{const a=fe(r);return a===null?{status:U}:i.utimes(n,a.atime,a.mtime,(s&$t)===0)}case c.SYMLINK:return i.symlink(r?new TextDecoder().decode(r):"",n);case c.OPEN:return i.open(n,s,t,Ie(r,je));case c.CLOSE:{const a=J(r);return a===null?{status:U}:i.close(a)}case c.FREAD:{const a=he(r);return a===null?{status:U}:i.fread(a.fd,a.length,a.position)}case c.FWRITE:{const a=me(r);return a===null?{status:U}:i.fwrite(a.fd,a.bytes,a.position)}case c.FSTAT:{const a=J(r);return a===null?{status:U}:i.fstat(a)}case c.FTRUNCATE:{const a=we(r);return a===null?{status:U}:i.ftruncate(a.fd,a.len)}case c.FCHMOD:{const a=ye(r);return a===null?{status:U}:i.fchmod(a.fd,a.mode)}case c.FCHOWN:{const a=pe(r);return a===null?{status:U}:i.fchown(a.fd,a.uid,a.gid)}case c.FUTIMES:{const a=be(r);return a===null?{status:U}:i.futimes(a.fd,a.atime,a.mtime)}default:return{status:U}}}var Ls=new Set([c.READ,c.WRITE,c.APPEND,c.UNLINK,c.STAT,c.LSTAT,c.MKDIR,c.RMDIR,c.READDIR,c.RENAME,c.EXISTS,c.TRUNCATE,c.COPY,c.ACCESS,c.REALPATH,c.CHMOD,c.CHOWN,c.UTIMES,c.SYMLINK,c.READLINK,c.LINK,c.OPEN,c.CLOSE,c.FREAD,c.FWRITE,c.FSTAT,c.FTRUNCATE,c.FSYNC,c.OPENDIR,c.MKDTEMP,c.FCHMOD,c.FCHOWN,c.FUTIMES,c.STATFS]),Kt=new WeakMap;function ts(i,t){const e=URL.createObjectURL(new Blob([i],{type:"text/javascript"}));try{const s=new Worker(e,{type:"module",name:t});return Kt.set(s,e),s}catch(s){throw URL.revokeObjectURL(e),s}}function es(i){if(!i)return;try{i.terminate()}catch{}const t=Kt.get(i);t&&(URL.revokeObjectURL(t),Kt.delete(i))}var ss=`function E(e){if(e.startsWith("/")||(e="/"+e),e.length===1)return e;const t=[];for(const r of e.split("/"))if(!(r===""||r===".")){if(r===".."){t.pop();continue}t.push(r)}return"/"+t.join("/")}function j(e,t){const r=E(t);for(let n=e.length-1;n>=0;n--){const a=e[n];if(E(a.path)!==r){if(a.op==="rename"&&a.newPath&&E(a.newPath)===r)return-1;continue}return a.op==="write"?n:-1}return-1}var h,m;function f(e){if(e.charCodeAt(0)!==47&&(e="/"+e),e.indexOf("//")!==-1&&(e=e.replace(/\\\\/\\\\/+/g,"/")),e.indexOf("/.")!==-1){const t=e.split("/"),r=[];for(const n of t)if(!(n==="."||n==="")){if(n===".."){r.pop();continue}r.push(n)}e="/"+r.join("/")}return e.length>1&&e.charCodeAt(e.length-1)===47&&(e=e.slice(0,-1)),e||"/"}function H(e){return f(e).split("/").filter(Boolean)}var D=new Set,p=new Map,M=new Map;function O(e){let t=2166136261;for(let r=0;r<e.length;r++)t^=e[r],t=Math.imul(t,16777619);return t>>>0^e.length}function P(e,t){if(e=f(e),D.has(e))return!0;const r=p.get(e);return!r||Date.now()-r>=A?!1:M.get(e)===O(t)}var A=3e3;function R(e){D.add(f(e))}function T(e){D.delete(f(e))}function z(e){p.set(f(e),Date.now())}function b(e,t=!1){e=f(e);const r=Date.now();if(D.has(e))return!0;const n=p.get(e);if(n&&r-n<A)return!0;if(t){let a=e;for(;;){const o=a.lastIndexOf("/");if(o<=0)break;if(a=a.substring(0,o),D.has(a))return!0;const i=p.get(a);if(i&&r-i<A)return!0}}return!1}setInterval(()=>{const e=Date.now()-A;for(const[t,r]of p)r<e&&(p.delete(t),M.delete(t))},5e3);var g=[],F=!1;function q(e){if(R(e.path),e.op==="rename"&&e.newPath&&R(e.newPath),e.op==="write"){const t=j(g,e.path);if(t!==-1){g[t].data=e.data,g[t].ts=e.ts;return}}g.push(e),F||I()}async function G(e){switch(e.op){case"write":e.data?await B(e.path,e.data):await B(e.path,new ArrayBuffer(0));break;case"delete":await K(e.path);break;case"mkdir":await X(e.path);break;case"rename":await J(e.path,e.newPath);break}}var C=4;async function I(){if(g.length===0){F=!1;return}F=!0;const e=g.shift();for(let t=1;t<=C;t++)try{await G(e);break}catch(r){if(t===C){console.warn("[opfs-sync] mirror failed after retries:",e.op,e.path,r);break}await new Promise(n=>setTimeout(n,10*t))}T(e.path),z(e.path),e.op==="rename"&&e.newPath&&(T(e.newPath),z(e.newPath)),I()}async function S(e){const t=H(e);t.pop();let r=m;for(const n of t)r=await r.getDirectoryHandle(n,{create:!0});return r}function y(e){const t=H(e);return t[t.length-1]||""}async function B(e,t){const r=await S(e),n=y(e),a=await r.getFileHandle(n,{create:!0}),o=new Uint8Array(t),i=await a.createSyncAccessHandle();try{i.truncate(0),i.write(o,{at:0}),i.flush()}finally{i.close()}M.set(f(e),O(o))}async function K(e){try{await(await x(e)).removeEntry(y(e),{recursive:!0})}catch{}}async function X(e){let t=m;for(const r of H(e))t=await t.getDirectoryHandle(r,{create:!0})}var U=2*1024*1024;async function J(e,t){let r=null,n=null,a,o,i=!1,u=null;for(let s=0;s<6;s++)try{a=await x(e),o=await a.getFileHandle(y(e)),i=!0;break}catch(d){u=d;const l=d?.message||"";if(d?.name==="TypeMismatchError"||l.includes("TypeMismatch")||l.includes("not a file")||l.includes("not an entry of requested type")){try{await W(e,t)}catch(c){console.warn("[opfs-sync] rename (dir) failed:",e,"\\\\u2192",t,c)}return}if(s<5){await new Promise(c=>setTimeout(c,8*(s+1)));continue}}if(!i){try{await W(e,t);return}catch{}console.warn("[opfs-sync] rename failed (source not found after retries):",e,"\\\\u2192",t,u);return}try{r=await o.createSyncAccessHandle();const s=r.getSize();if(n=await(await(await S(t)).getFileHandle(y(t),{create:!0})).createSyncAccessHandle(),n.truncate(0),s>0){const w=new Uint8Array(Math.min(s,U));let c=0;for(;c<s;){const k=Math.min(w.length,s-c),v=k===w.length?w:w.subarray(0,k);r.read(v,{at:c}),n.write(v,{at:c}),c+=k}}n.flush();try{n.close()}catch{}n=null;try{r.close()}catch{}r=null,await N(a,y(e))}catch(s){console.warn("[opfs-sync] rename failed:",e,"\\\\u2192",t,s)}finally{if(n)try{n.close()}catch{}if(r)try{r.close()}catch{}}}async function W(e,t){const r=await x(e),n=await r.getDirectoryHandle(y(e)),a=await S(t);try{await a.removeEntry(y(t),{recursive:!0})}catch(i){if(i?.name!=="NotFoundError")throw i}const o=await a.getDirectoryHandle(y(t),{create:!0});await _(n,o),await N(r,y(e),{recursive:!0})}async function N(e,t,r){let n=null;for(let a=0;a<4;a++)try{await e.removeEntry(t,r);return}catch(o){if(o?.name==="NotFoundError")return;n=o,await new Promise(i=>setTimeout(i,10*(a+1)))}throw n}async function _(e,t){for await(const[r,n]of e.entries())if(n.kind==="directory"){const a=await t.getDirectoryHandle(r,{create:!0});await _(n,a)}else{const a=n,o=await t.getFileHandle(r,{create:!0});let i=null,u=null;try{i=await a.createSyncAccessHandle(),u=await o.createSyncAccessHandle();const s=i.getSize();if(u.truncate(0),s>0){const d=new Uint8Array(Math.min(s,U));let l=0;for(;l<s;){const w=Math.min(d.length,s-l),c=w===d.length?d:d.subarray(0,w);i.read(c,{at:l}),u.write(c,{at:l}),l+=w}}u.flush()}finally{if(u)try{u.close()}catch{}if(i)try{i.close()}catch{}}}}async function x(e){const t=H(e);t.pop();let r=m;for(const n of t)r=await r.getDirectoryHandle(n);return r}function L(e){for(const t of e){const r=f(t.path);if(!(r==="/.vfs.bin"||r==="/.vfs"||r.startsWith("/.vfs")))switch(t.kind){case"appeared":case"modified":Q(r,t.handle??null);break;case"disappeared":if(b(r,!0))continue;V(r);break;case"moved":{const n=f(t.from);if(b(r)||b(n))continue;Y(n,r);break}}}}async function Q(e,t){try{if(!t||t.kind!=="file")return;const r=t;let n=await r.getFile().then(a=>a.arrayBuffer());if(P(e,new Uint8Array(n))||p.has(f(e))&&(n=await r.getFile().then(a=>a.arrayBuffer()),P(e,new Uint8Array(n))))return;h.postMessage({op:"external-write",path:e,data:n,ts:Date.now()},[n])}catch(r){console.warn("[opfs-sync] external change read failed:",e,r)}}function V(e){h.postMessage({op:"external-delete",path:e,ts:Date.now()})}function Y(e,t){h.postMessage({op:"external-rename",path:e,newPath:t,ts:Date.now()})}self.onmessage=async e=>{const t=e.data;if(t.type==="init"){if(h=e.ports[0],m=await navigator.storage.getDirectory(),t.root&&t.root!=="/"){const r=t.root.split("/").filter(Boolean);for(const n of r)m=await m.getDirectoryHandle(n,{create:!0})}console.log("[opfs-sync] initialized with root:",t.root||"/","mirrorRoot.name:",m.name||"(opfs-root)"),h.onmessage=r=>{if(r.data?.type==="external-records"){L(r.data.records);return}q(r.data)},h.start(),self.postMessage({type:"ready"});return}if(t.type==="shutdown"){try{h?.close()}catch{}self.postMessage({type:"shutdown-done"}),self.close();return}};\n`;function ns(i,t,e){const s=(t&64)!==0,n=(t&512)!==0,r=s&&!n?i.exists(e).data?.[0]===1:!1;return{willCreate:s,willTrunc:n,existedBefore:r}}var Ct=(i,t)=>new DataView(i.buffer,i.byteOffset,i.byteLength).getUint32(t,!0);function is(i,t,e,s,n,r){if(n.status!==0)return null;switch(t){case c.WRITE:case c.APPEND:case c.UNLINK:case c.MKDIR:case c.RMDIR:case c.TRUNCATE:case c.CHMOD:case c.CHOWN:case c.UTIMES:case c.SYMLINK:return{op:t,path:e};case c.RENAME:return{op:t,path:e,newPath:s?G(s):""};case c.COPY:case c.LINK:return{op:t,path:s?G(s):""};case c.OPEN:return r&&(r.willTrunc||r.willCreate&&!r.existedBefore)?{op:c.WRITE,path:e}:null;case c.MKDTEMP:return n.data instanceof Uint8Array?{op:t,path:new TextDecoder().decode(n.data)}:null;case c.FWRITE:case c.FTRUNCATE:return s&&s.byteLength>=4?{op:t,path:i.getPathForFd(Ct(s,0))??void 0}:null;case c.FCHMOD:return s&&s.byteLength>=4?{op:c.CHMOD,path:i.getPathForFd(Ct(s,0))??void 0}:null;case c.FCHOWN:return s&&s.byteLength>=4?{op:c.CHOWN,path:i.getPathForFd(Ct(s,0))??void 0}:null;case c.FUTIMES:return s&&s.byteLength>=4?{op:c.UTIMES,path:i.getPathForFd(Ct(s,0))??void 0}:null;default:return null}}self.addEventListener("error",i=>{console.error("[sync-relay] uncaught error:",i.message,i.filename,i.lineno)}),self.addEventListener("unhandledrejection",i=>{const t=i.reason;console.error("[sync-relay] unhandled rejection:",t?.message??String(t),t?.stack??"")});var k=new Be;function Ee(i,t){return!i||i.byteLength<4?t:new DataView(i.buffer,i.byteOffset,i.byteLength).getUint32(0,!0)}var Ft=null,tt=!1,Z=!1,q=!1,B=!1,Rt=!1,v=null,Vt=!1,dt=null,ft=null,x,T,et,z,_=null,A=null,N="",H=j.HEADER_SIZE,rs=j.HEARTBEAT>>2,as=1e3,ge=null;function Yt(){ge!==null||!T||(ge=setInterval(()=>{Atomics.add(T,rs,1)},as))}var ht=new Map,Q=[],Se=typeof navigator<"u"&&navigator.userAgent||"",os=/AppleWebKit/.test(Se)&&!/Chrome|Chromium|Android|Edg|OPR/.test(Se),Te=void 0;function mt(){const i=self.__fs_force_spin??Te;return i===void 0?os:!!i}var Wt=new MessageChannel;Wt.port2.start();function wt(){return new Promise(i=>{let t=!1,e=null;const s=()=>{t||(t=!0,e!==null&&clearTimeout(e),i())};Wt.port2.onmessage=s,Wt.port1.postMessage(null),mt()&&(e=setTimeout(s,1))})}function yt(i,t){try{return ws(i,t)}catch(e){return console.error("[sync-relay] handleRequest threw:",e?.message,e?.stack),{status:Y.EIO}}}async function pt(i,t){try{return await ys(i,t)}catch(e){return console.error("[sync-relay] handleRequestOPFS threw:",e?.message,e?.stack),{status:Y.EIO}}}function cs(i,t){t.onmessage=async e=>{if(e.data.buffer instanceof ArrayBuffer)if(Rt)Q.push({port:t,tabId:i,id:e.data.id,buffer:e.data.buffer});else{const s=tt?await pt(i,e.data.buffer):yt(i,e.data.buffer),n=W(s.status,s.data);t.postMessage({id:e.data.id,buffer:n},[n]),!tt&&s._op!==void 0&&St(s._op,s._path,s._newPath)}},t.start(),ht.set(i,t)}function ls(i){const t=ht.get(i);t&&(t.close(),ht.delete(i)),tt?Ft?.cleanupTab(i):k.cleanupTab(i)}function us(){for(;Q.length>0;){const i=Q.shift(),t=yt(i.tabId,i.buffer),e=W(t.status,t.data);i.port.postMessage({id:i.id,buffer:e},[e]),t._op!==void 0&&St(t._op,t._path,t._newPath)}}async function ds(){for(;Q.length>0;){const i=Q.shift(),t=await pt(i.tabId,i.buffer),e=W(t.status,t.data);i.port.postMessage({id:i.id,buffer:e},[e])}}var st=new Ve(()=>N,void 0,()=>{self.postMessage({type:"need-leader"})}),Gt=null;function fs(i){return st.forward(i)}function hs(i){if(i.data.buffer instanceof ArrayBuffer){if(st.handleResponse(i.data.id,i.data.buffer))return;Gt&&Gt.postMessage({id:i.data.id,buffer:i.data.buffer},[i.data.buffer])}}var ms={1:"READ",2:"WRITE",3:"UNLINK",4:"STAT",5:"LSTAT",6:"MKDIR",7:"RMDIR",8:"READDIR",9:"RENAME",10:"EXISTS",11:"TRUNCATE",12:"APPEND",13:"COPY",14:"ACCESS",15:"REALPATH",16:"CHMOD",17:"CHOWN",18:"UTIMES",19:"SYMLINK",20:"READLINK",21:"LINK",22:"OPEN",23:"CLOSE",24:"FREAD",25:"FWRITE",26:"FSTAT",27:"FTRUNCATE",28:"FSYNC",29:"OPENDIR",30:"MKDTEMP"};function ws(i,t){const e=B?performance.now():0;let s,n,r,a;try{({op:s,flags:n,path:r,data:a}=ae(t))}catch(E){return console.error(`[sync-relay] decodeRequest failed (bufLen=${t.byteLength}): ${E.message}`),{status:-1}}const o=B?performance.now():0,l=s===c.OPEN?ns(k,n,r):void 0,f=Je(k,i,s,n,r,a),h=is(k,s,r,a,f,l),d=h?.op,m=h?.path,w=h?.newPath;if(B){const E=performance.now();console.log(`[sync-relay] op=${ms[s]??s} path=${r} decode=${(o-e).toFixed(3)}ms engine=${(E-o).toFixed(3)}ms TOTAL=${(E-e).toFixed(3)}ms`)}const u={status:f.status,data:f.data instanceof Uint8Array?f.data:void 0};return d!==void 0&&m&&(u._op=d,u._path=m,u._newPath=w,Et(d,m,w)),u}async function ys(i,t){const e=Ft;let s,n,r,a;try{({op:s,flags:n,path:r,data:a}=ae(t))}catch(u){return console.error(`[sync-relay] decodeRequest failed in OPFS handler (bufLen=${t.byteLength}): ${u.message}`),{status:-1}}let o,l,f;switch(s){case c.READ:o=await e.read(r);break;case c.WRITE:o=await e.write(r,a??new Uint8Array(0),n),l=r;break;case c.APPEND:o=await e.append(r,a??new Uint8Array(0)),l=r;break;case c.UNLINK:o=await e.unlink(r),l=r;break;case c.STAT:o=await e.stat(r);break;case c.LSTAT:o=await e.lstat(r);break;case c.MKDIR:o=await e.mkdir(r,n,Ee(a,511)),l=r;break;case c.RMDIR:o=await e.rmdir(r,n),l=r;break;case c.READDIR:o=await e.readdir(r,n);break;case c.RENAME:{const u=a?G(a):"";o=await e.rename(r,u),l=r,f=u;break}case c.EXISTS:o=await e.exists(r);break;case c.TRUNCATE:{const u=ue(a);if(u===null){o={status:7};break}o=await e.truncate(r,u),l=r;break}case c.COPY:{const u=a?G(a):"";o=await e.copy(r,u,n),l=u;break}case c.ACCESS:o=await e.access(r,n);break;case c.REALPATH:o=await e.realpath(r);break;case c.CHMOD:{const u=Ht(a)??0;o=await e.chmod(r,u);break}case c.CHOWN:{const u=de(a);if(u===null){o={status:7};break}o=await e.chown(r,u.uid,u.gid);break}case c.UTIMES:{const u=fe(a);if(u===null){o={status:7};break}o=await e.utimes(r,u.atime,u.mtime);break}case c.SYMLINK:{const u=a?new TextDecoder().decode(a):"";o=await e.symlink(u,r);break}case c.READLINK:o=await e.readlink(r);break;case c.LINK:{const u=a?G(a):"";o=await e.link(r,u),l=u;break}case c.OPEN:o=await e.open(r,n,i,Ee(a,438));break;case c.CLOSE:{const u=J(a);if(u===null){o={status:7};break}o=await e.close(u);break}case c.FREAD:{const u=he(a);if(u===null){o={status:7};break}o=await e.fread(u.fd,u.length,u.position);break}case c.FWRITE:{const u=me(a);if(u===null){o={status:7};break}o=await e.fwrite(u.fd,u.bytes,u.position),l=e.getPathForFd(u.fd)??void 0;break}case c.FSTAT:{const u=J(a);if(u===null){o={status:7};break}o=await e.fstat(u);break}case c.FTRUNCATE:{const u=we(a);if(u===null){o={status:7};break}o=await e.ftruncate(u.fd,u.len),l=e.getPathForFd(u.fd)??void 0;break}case c.FSYNC:o=await e.fsync(J(a)??void 0);break;case c.STATFS:o=await e.statfs();break;case c.OPENDIR:o=await e.opendir(r,i);break;case c.MKDTEMP:o=await e.mkdtemp(r),o.status===0&&o.data&&(l=new TextDecoder().decode(o.data instanceof Uint8Array?o.data:new Uint8Array(0)));break;case c.FCHMOD:{const u=ye(a);if(u===null){o={status:7};break}o=await e.fchmod(u.fd,u.mode);break}case c.FCHOWN:{const u=pe(a);if(u===null){o={status:7};break}o=await e.fchown(u.fd,u.uid,u.gid);break}case c.FUTIMES:{const u=be(a);if(u===null){o={status:7};break}o=await e.futimes(u.fd,u.atime,u.mtime);break}default:o={status:7}}const h=1,d=[c.READ,c.STAT,c.LSTAT,c.READDIR,c.EXISTS,c.ACCESS,c.REALPATH,c.READLINK],m=s===c.EXISTS&&o.status===0&&o.data instanceof Uint8Array&&o.data[0]===0;if((o.status===h||m)&&d.includes(s)){const u=(()=>{switch(s){case c.READ:return k.read(r);case c.STAT:return k.stat(r);case c.LSTAT:return k.lstat(r);case c.READDIR:return k.readdir(r,n);case c.EXISTS:return k.exists(r);case c.ACCESS:return k.access(r,n);case c.REALPATH:return k.realpath(r);case c.READLINK:return k.readlink(r);default:return null}})();u&&u.status!==h&&(o=u)}const w={status:o.status,data:o.data instanceof Uint8Array?o.data:void 0};return o.status===0&&l&&Et(s,l,f),w}function nt(i,t){const e=new BigUint64Array(i,j.TOTAL_LEN,1),s=i.byteLength-H,n=Atomics.load(t,3),r=Number(Atomics.load(e,0));if(n<=0||n>s)return console.error(`[sync-relay] readPayload: invalid chunkLen=${n} (maxChunk=${s}, totalLen=${r})`),new Uint8Array(0);if(r<=s)return new Uint8Array(i,H,n).slice();if(r>It.maxPayload||r<=0)return console.error(`[sync-relay] readPayload: totalLen=${r} exceeds limit (${It.maxPayload}) or invalid`),new Uint8Array(0);const a=new Uint8Array(r);let o=0;a.set(new Uint8Array(i,H,n),o),o+=n;const l=Date.now()+oe;for(;o<r;){Atomics.store(t,0,S.CHUNK_ACK),Atomics.notify(t,0),ce(t,S.CHUNK_ACK,l,"request chunk from caller",50);const f=Atomics.load(t,3);if(f<=0||f>s)return console.error(`[sync-relay] readPayload: invalid nextLen=${f} at offset=${o}`),a.slice(0,o);a.set(new Uint8Array(i,H,f),o),o+=f}return a}function it(i,t,e,s){const n=s?s.byteLength:0,r=8+n,a=i.byteLength-H;if(r<=a){const o=new DataView(i,H,8);o.setUint32(0,e,!0),o.setUint32(4,n,!0),s&&n>0&&new Uint8Array(i,H+8,n).set(s),Atomics.store(t,3,r);const l=new BigUint64Array(i,j.TOTAL_LEN,1);Atomics.store(l,0,BigInt(r)),Atomics.store(t,0,S.RESPONSE),Atomics.notify(t,0)}else{const o=W(e,s);Zt(i,t,new Uint8Array(o))}}function Zt(i,t,e){const s=i.byteLength-H;if(e.byteLength<=s){new Uint8Array(i,H,e.byteLength).set(e),Atomics.store(t,3,e.byteLength);const n=new BigUint64Array(i,j.TOTAL_LEN,1);Atomics.store(n,0,BigInt(e.byteLength)),Atomics.store(t,0,S.RESPONSE),Atomics.notify(t,0)}else{const n=new BigUint64Array(i,j.TOTAL_LEN,1);Atomics.store(n,0,BigInt(e.byteLength));let r=0;for(;r<e.byteLength;){const a=Math.min(s,e.byteLength-r);new Uint8Array(i,H,a).set(e.subarray(r,r+a)),Atomics.store(t,3,a),Atomics.store(t,6,Math.floor(r/s));const o=r+a>=e.byteLength;Atomics.store(t,0,o?S.RESPONSE:S.CHUNK),Atomics.notify(t,0),o||ce(t,S.CHUNK,Date.now()+oe,"response chunk ack from caller",50),r+=a}}}var ps=25,vt=0;function bs(){if(mt())try{if(Date.now()-vt<ps||Atomics.load(T,0)===S.REQUEST||A&&Atomics.load(A,0)===S.REQUEST)return;k.maybePreGrow(!0)}catch(i){console.error("[sync-relay] pre-grow failed:",i?.message)}}function rt(i,t){if(!mt())return Atomics.wait(i,0,S.RESPONSE,t),Atomics.load(i,0)!==S.RESPONSE;const e=Date.now()+t;for(;Atomics.load(i,0)===S.RESPONSE;){const s=e-Date.now();if(s<=0)return!1;Atomics.wait(i,0,S.RESPONSE,Math.min(5,s))}return!0}var Is=0,Es=5;function qt(i,t){i().catch(e=>{Rt=!1,console.error(`[sync-relay] ${t} crashed:`,e?.message,e?.stack);try{T&&Atomics.load(T,0)===S.REQUEST&&it(x,T,Y.EIO),A&&Atomics.load(A,0)===S.REQUEST&&it(_,A,Y.EIO)}catch{}++Is<=Es?qt(i,t):self.postMessage({type:"leader-loop-fatal",error:e?.message??String(e)})})}async function gs(){for(Rt=!0;;){let i=!0,t=0;for(;i;){if(i=!1,++t>=100&&(t=0,await wt()),Atomics.load(T,0)===S.REQUEST){const e=B?performance.now():0,s=nt(x,T),n=B?performance.now():0,r=yt(N,s.buffer),a=B?performance.now():0;it(x,T,r.status,r.data),r._op!==void 0&&St(r._op,r._path,r._newPath);const o=B?performance.now():0;B&&console.log(`[leaderLoop] readPayload=${(n-e).toFixed(3)}ms handleRequest=${(a-n).toFixed(3)}ms writeResponse=${(o-a).toFixed(3)}ms TOTAL=${(o-e).toFixed(3)}ms`),rt(T,100)||Atomics.store(T,0,S.IDLE),vt=Date.now(),i=!0;continue}if(A&&Atomics.load(A,0)===S.REQUEST){const e=nt(_,A),s=yt(N,e.buffer);it(_,A,s.status,s.data),s._op!==void 0&&St(s._op,s._path,s._newPath),rt(A,5e3),vt=Date.now(),i=!0;continue}if(Q.length>0){us(),i=!0;continue}if(mt()&&Date.now()-vt<20){const e=performance.now();for(;performance.now()-e<.25;)if(Atomics.load(T,0)===S.REQUEST||A!==null&&Atomics.load(A,0)===S.REQUEST){i=!0;break}}}if(await wt(),bs(),ht.size===0){const e=Atomics.load(T,0),s=A!==null&&Atomics.load(A,0)===S.REQUEST;e!==S.REQUEST&&!s&&Atomics.wait(T,0,e,Vt?5:50)}}}async function Ss(){for(Rt=!0;;){let i=!0,t=0;for(;i;){if(i=!1,++t>=100&&(t=0,await wt()),Atomics.load(T,0)===S.REQUEST){const e=nt(x,T),s=await pt(N,e.buffer);it(x,T,s.status,s.data),rt(T,100)||Atomics.store(T,0,S.IDLE),i=!0;continue}if(A&&Atomics.load(A,0)===S.REQUEST){const e=nt(_,A),s=await pt(N,e.buffer);it(_,A,s.status,s.data),rt(A,100)||Atomics.store(A,0,S.IDLE),i=!0;continue}if(Q.length>0){await ds(),i=!0;continue}}if(await wt(),ht.size===0){const e=Atomics.load(T,0),s=A!==null&&Atomics.load(A,0)===S.REQUEST;e!==S.REQUEST&&!s&&Atomics.wait(T,0,e,50)}}}async function Ts(){for(;;){if(Atomics.load(T,0)===S.REQUEST){const t=nt(x,T),e=await st.forward(t,!0);Zt(x,T,new Uint8Array(e)),rt(T,100)||Atomics.store(T,0,S.IDLE);continue}if(A&&Atomics.load(A,0)===S.REQUEST){const t=nt(_,A),e=await fs(t);Zt(_,A,new Uint8Array(e)),rt(A,100)||Atomics.store(A,0,S.IDLE);continue}Atomics.wait(T,0,S.IDLE,50)==="timed-out"&&await wt()}}var As=new Set([".vfs.bin",".vfs.bin.tmp"]);async function Ds(i){const s=Date.now();for(;;)try{return await i.createSyncAccessHandle()}catch(n){const r=n?.name;if(r!=="NoModificationAllowedError"&&r!=="InvalidStateError"||Date.now()-s>=3e3)throw n;await new Promise(a=>setTimeout(a,50))}}var bt=null,Ae=!1,ks=2*1024*1024;async function De(i,t){const e=[],s=[];for await(const[n,r]of i.entries())t===""&&As.has(n)||(r.kind==="directory"?e.push({name:n,handle:r}):s.push({name:n,handle:r}));for(const{name:n}of e){const r=t?`${t}/${n}`:`/${n}`;k.mkdir(r,1,493)}for(const{name:n,handle:r}of s){const a=t?`${t}/${n}`:`/${n}`;let o=null;try{o=await r.createSyncAccessHandle();const l=o.getSize();if(k.write(a,new Uint8Array(0)),l>0){const f=new Uint8Array(Math.min(l,ks));let h=0;for(;h<l;){const d=Math.min(f.length,l-h),m=d===f.length?f:f.subarray(0,d);o.read(m,{at:h}),k.append(a,m),h+=d}}}finally{if(o)try{o.close()}catch{}}}for(const{name:n,handle:r}of e){const a=t?`${t}/${n}`:`/${n}`;await De(r,a)}}var ke={maxInodes:4e6,maxBlocks:4e6,maxPathTable:256*1024*1024,maxVFSSize:100*1024*1024*1024,maxPayload:2*1024*1024*1024};function Os(i){return{...ke,...i}}var It={...ke};function Cs(i,t,e){if(t<b.SIZE)return`file too small (${t} bytes)`;const s=new Uint8Array(b.SIZE);i.read(s,{at:0});const n=new DataView(s.buffer),r=n.getUint32(b.MAGIC,!0);if(r!==lt)return`bad magic 0x${r.toString(16)}`;const a=n.getUint32(b.VERSION,!0);if(a!==At)return`unsupported version ${a}`;const o=n.getUint32(b.INODE_COUNT,!0),l=n.getUint32(b.BLOCK_SIZE,!0),f=n.getUint32(b.TOTAL_BLOCKS,!0),h=n.getUint32(b.FREE_BLOCKS,!0),d=n.getFloat64(b.INODE_OFFSET,!0),m=n.getFloat64(b.PATH_OFFSET,!0),w=n.getFloat64(b.DATA_OFFSET,!0),u=n.getFloat64(b.BITMAP_OFFSET,!0),E=n.getUint32(b.PATH_USED,!0);if(l===0||(l&l-1)!==0)return`invalid block size ${l}`;if(o===0)return"inode count is 0";if(o>e.maxInodes)return`inode count ${o} exceeds maximum ${e.maxInodes}`;if(f>e.maxBlocks)return`total blocks ${f} exceeds maximum ${e.maxBlocks}`;if(h>f)return`free blocks (${h}) exceeds total (${f})`;if(!Number.isFinite(d)||d<0||!Number.isFinite(m)||m<0||!Number.isFinite(u)||u<0||!Number.isFinite(w)||w<0)return"non-finite or negative section offset";if(d!==b.SIZE)return`inode table offset ${d} (expected ${b.SIZE})`;const D=d+o*P;if(m!==D)return`path table offset ${m} (expected ${D})`;if(u<=m)return"bitmap offset must be after path table";if(w<=u)return"data offset must be after bitmap";const C=u-m;if(E>C)return`path used (${E}) exceeds path table size (${C})`;if(C>e.maxPathTable)return`path table size ${C} exceeds maximum ${e.maxPathTable}`;const F=w+f*l;return F>e.maxVFSSize?`computed layout size ${F} exceeds maximum ${e.maxVFSSize}`:t<F?`file size ${t} too small for layout (need ${F})`:null}async function Fs(i){B=i.debug??!1,Te=i.forceSpin,It=Os(i.limits);let t=await navigator.storage.getDirectory();if(i.root&&i.root!=="/"){const a=i.root.split("/").filter(Boolean);for(const o of a)t=await t.getDirectoryHandle(o,{create:!0})}const e=await t.getFileHandle(".vfs.bin",{create:!0}),s=await Ds(e);bt=s;const n=s.getSize();if(n>0){const a=Cs(s,n,It);if(a){try{s.close()}catch{}throw bt=null,new Error(`Corrupt VFS: ${a}`)}}const r=n===0;try{k.init(s,{uid:i.uid,gid:i.gid,umask:i.umask,strictPermissions:i.strictPermissions,debug:i.debug,limits:It})}catch(a){try{s.close()}catch{}throw a}if(r&&(await De(t,""),k.flush()),i.opfsSync){Vt=!0;const a=new MessageChannel;v=a.port1,v.onmessage=o=>xs(o.data),v.start(),dt=ts(ss,"vfs-opfs-sync"),dt.postMessage({type:"init",root:i.opfsSyncRoot??i.root},[a.port2])}if(mt())try{k.maybePreGrow(!0)}catch(a){console.error("[sync-relay] init pre-grow failed:",a?.message)}ft=new BroadcastChannel(`${i.ns}-watch`)}async function Rs(i){B=i.debug??!1,tt=!0;let t=await navigator.storage.getDirectory();if(i.root&&i.root!=="/"){const e=i.root.split("/").filter(Boolean);for(const s of e)t=await t.getDirectoryHandle(s,{create:!0})}Ft=new He,await Ft.init(t,{uid:i.uid,gid:i.gid}),ft=new BroadcastChannel(`${i.ns}-watch`)}function Et(i,t,e){if(!ft)return;let s;switch(i){case c.WRITE:case c.APPEND:case c.TRUNCATE:case c.FWRITE:case c.FTRUNCATE:case c.CHMOD:case c.CHOWN:case c.UTIMES:case c.COPY:s="change";break;case c.UNLINK:case c.RMDIR:case c.RENAME:case c.MKDIR:case c.MKDTEMP:case c.SYMLINK:case c.LINK:s="rename";break;default:return}ft.postMessage({eventType:s,path:t}),i===c.RENAME&&e&&ft.postMessage({eventType:"rename",path:e})}var at=new Map,vs=50,ot=new Map,ct=new Map;function Oe(i){const t=k.readlink(i);if(t.status!==0||!t.data)return;const e=qe(i,new TextDecoder().decode(t.data));Qe(ot,ct,i,e)}function Us(i){if(at.delete(i),!!v)try{const t=k.read(i);if(t.status!==0){k.readlink(i).status===0&&v.postMessage({op:"write",path:i,data:new ArrayBuffer(0),ts:Date.now()});return}const e=Date.now();if(t.data&&t.data.byteLength>0){const s=t.data.buffer.byteLength===t.data.byteLength?t.data.buffer:t.data.slice().buffer;v.postMessage({op:"write",path:i,data:s,ts:e},[s])}else v.postMessage({op:"write",path:i,data:new ArrayBuffer(0),ts:e});gt(i)}catch{}}function gt(i){const t=ot.get(i);if(t)for(const e of t)xt(e)}function Ut(i){gt(i);for(const t of _t(ot.keys(),i))t!==i&&gt(t)}function X(i){const t=at.get(i);t&&(clearTimeout(t),at.delete(i))}function Ce(i,t){for(const{from:e,to:s}of Ze(at.keys(),i,t))X(e),xt(s)}function Fe(i,t){for(const e of _t(ct.keys(),i))Ot(ot,ct,e),Oe(e===i?t:t+e.slice(i.length))}function Re(i,t){if(t)for(const e of _t(ct.keys(),i))Ot(ot,ct,e);else Ot(ot,ct,i)}function xt(i){const t=at.get(i);t&&clearTimeout(t),at.set(i,setTimeout(()=>Us(i),vs))}function St(i,t,e){if(!v)return;const s=Date.now();switch(i){case c.WRITE:case c.APPEND:case c.TRUNCATE:case c.FWRITE:case c.FTRUNCATE:case c.COPY:case c.LINK:{xt(t),gt(t);break}case c.SYMLINK:{Oe(t),xt(t);break}case c.UNLINK:case c.RMDIR:{X(t),Re(t,i===c.RMDIR),v.postMessage({op:"delete",path:t,ts:s}),Ut(t);break}case c.MKDIR:case c.MKDTEMP:v.postMessage({op:"mkdir",path:t,ts:s});break;case c.RENAME:if(e){X(t),X(e),Ce(t,e),Fe(t,e);const n=Ge(k,t,e,s);for(const r of n.messages)r.op==="write"&&n.transfers.includes(r.data)?v.postMessage(r,[r.data]):v.postMessage(r);Ut(t)}break}}function xs(i){if(!Ae)switch(i.op){case"external-write":{let t=k.write(i.path,new Uint8Array(i.data),0);t.status===Y.EISDIR&&(k.rmdir(i.path,1),t=k.write(i.path,new Uint8Array(i.data),0)),t.status===0&&(gt(i.path),Et(c.WRITE,i.path)),console.log("[sync-relay] external-write:",i.path,`${i.data?.byteLength??0}B`,`status=${t.status}`);break}case"external-delete":{X(i.path);let t=!1,e=k.unlink(i.path).status===0;e||(e=k.rmdir(i.path,1).status===0,t=e),e&&(Re(i.path,t),Ut(i.path),Et(t?c.RMDIR:c.UNLINK,i.path)),console.log("[sync-relay] external-delete:",i.path,`dir=${t}`,`ok=${e}`);break}case"external-rename":if(i.newPath){const t=k.rename(i.path,i.newPath);t.status===0&&(X(i.path),X(i.newPath),Ce(i.path,i.newPath),Fe(i.path,i.newPath),Ut(i.path),Et(c.RENAME,i.path,i.newPath)),console.log("[sync-relay] external-rename:",i.path,"\\u2192",i.newPath,`status=${t.status}`)}break}}self.onmessage=async i=>{const t=i.data;if(t.type==="async-port"){const e=t.port??i.ports[0];e&&(Gt=e,e.onmessage=async s=>{if(s.data.buffer instanceof ArrayBuffer){if(Z){const n=tt?await pt(N||"nosab",s.data.buffer):yt(N||"nosab",s.data.buffer),r=W(n.status,n.data);e.postMessage({id:s.data.id,buffer:r},[r]),!tt&&n._op!==void 0&&St(n._op,n._path,n._newPath)}else if(st.hasPort){const n=s.data.buffer;st.postRaw({id:s.data.id,tabId:N,buffer:n},[n])}}},e.start());return}if(t.type==="init-leader"){if(Z)return;Z=!0,N=t.tabId;const e=t.sab!=null;e&&(x=t.sab,et=t.readySab,T=new Int32Array(x,0,8),z=new Int32Array(et,0,1),Yt()),t.asyncSab&&(_=t.asyncSab,A=new Int32Array(t.asyncSab,0,8));try{await Fs(t.config)}catch(s){Z=!1,self.postMessage({type:"init-failed",error:s.message});return}q||(q=!0,e&&(Atomics.store(z,0,1),Atomics.notify(z,0)),self.postMessage({type:"ready"})),e&&qt(gs,"leaderLoop");return}if(t.type==="init-opfs"){Z=!0,q=!1,N=t.tabId;const e=t.sab!=null;e&&(x=t.sab,et=t.readySab,T=new Int32Array(x,0,8),z=new Int32Array(et,0,1),Yt()),t.asyncSab&&(_=t.asyncSab,A=new Int32Array(t.asyncSab,0,8));try{await Rs(t.config)}catch(s){Z=!1,self.postMessage({type:"init-failed",error:s.message});return}q||(q=!0,e&&(Atomics.store(z,0,1),Atomics.notify(z,0)),self.postMessage({type:"ready",mode:"opfs"})),e&&qt(Ss,"leaderLoopOPFS");return}if(t.type==="init-follower"){N=t.tabId,t.sab!=null&&(x=t.sab,et=t.readySab,T=new Int32Array(x,0,8),z=new Int32Array(et,0,1),Yt()),t.asyncSab&&(_=t.asyncSab,A=new Int32Array(t.asyncSab,0,8));return}if(t.type==="leader-port"){if(Z)return;const e=t.port??i.ports[0];if(!e)return;st.setPort(e),e.onmessage=hs,e.start(),q||(q=!0,z&&(Atomics.store(z,0,1),Atomics.notify(z,0)),self.postMessage({type:"ready"}),T&&Ts());return}if(t.type==="client-port"){cs(t.tabId,t.port??i.ports[0]);return}if(t.type==="client-lost"){ls(t.tabId);return}if(t.type==="external-records"){v?.postMessage({type:"external-records",records:t.records});return}if(t.type==="shutdown"){try{k.flush()}catch{}try{bt?.flush?.()}catch{}try{bt?.close()}catch{}if(bt=null,Ae=!0,dt){const e=dt;dt=null,await new Promise(s=>{const n=()=>{clearTimeout(r),es(e),s()},r=setTimeout(n,500);e.onmessage=a=>{a.data?.type==="shutdown-done"&&n()},e.postMessage({type:"shutdown"})})}try{v?.close()}catch{}v=null,Vt=!1,self.postMessage({type:"shutdown-done"});return}};\n';
