...
var VFSEngine = class _VFSEngine {
   ...
   */
  open(path, flags, tabId2, reqMode = 438) {
    path = this.normalizePath(path);
...
var OPFSEngine = class {
   ...
   */
  async mkdir(path, flags = 0, _reqMode = 511) {
    path = this.normalizePath(path);
    const recursive = (flags & 1) !== 0;
    if (recursive) {
      const parts = path.split("/").filter(Boolean);
   ...
   */
  async open(path, flags, _tabId, _reqMode = 438) {
    path = this.normalizePath(path);
    const hasCreate = (flags & 64) !== 0;
    const hasTrunc = (flags & 512) !== 0;
    const hasExcl = (flags & 128) !== 0;
...
function resyncHardlinksFor(path) {
