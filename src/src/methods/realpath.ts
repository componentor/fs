...
/**
 * Node's `fs.realpath(path[, options])`: `options` is an encoding name or `{ encoding }`, and
 * `'buffer'` asks for the raw bytes instead of a string. An unrecognised name is rejected rather
 * than silently ignored, exactly as it is for readdir and readlink.
 */
